def valid($disconnect):
  ...
  all(.results[];
    ...
    .login_save.disconnect_confirmed == $disconnect and
    .login_save.logout_confirmed == ($disconnect | not) and
    (if $disconnect then .login_save.login_account_offline == true else true end) and
    .login_save.offline == true and .login_save.retained_existing_rows == true and
...
if length != 2 or (.[0] | valid(true) | not) or (.[1] | valid(false) | not) then
  error("missing distinct transport-disconnect and normal-relogin evidence")
  ...
  error("saved projections changed across relog")
...
else .[1] + {login_disconnect_relog_verified: true} end
