...
impl PowerTypeStore {
    ...
    pub fn apply_hotfix_overlays_like_cpp(
        &mut self,
        official: impl IntoIterator<Item = PowerTypeEntry>,
        custom: impl IntoIterator<Item = PowerTypeEntry>,
    ) -> usize {
        let mut count = 0usize;
        for entry in official.into_iter().chain(custom) {
            self.overlay_hotfix_row_like_cpp(entry);
            count += 1;
        }
        count
    }
