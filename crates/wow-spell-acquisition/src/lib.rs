...
//! Deterministic application planning for player spell and skill acquisition.
...
//!
//! This is an application boundary, not a value-only domain crate: its borrowed
//! catalogs are the canonical read-only stores from `wow-data`. It depends on
//! neither Session nor database/network/packet APIs. Applying and persisting a
//! plan remains the caller's responsibility. The general spell-runtime crate
//! `wow-spell` remains a separate responsibility.
