...
pub(crate) fn set_difficulty_request(difficulty_id: u32) -> WorldPacket {
...
pub(crate) fn set_dungeon_difficulty_request(difficulty_id: u32) -> WorldPacket {
    let mut request = WorldPacket::new_empty();
    request.write_uint32(difficulty_id);
    request.reset_read();
    request
...
pub(crate) fn set_raid_difficulty_request(difficulty_id: i32, legacy: u8) -> WorldPacket {
    let mut request = WorldPacket::new_empty();
...
pub(crate) fn request_cemetery_list_packet(extra_payload: Option<u8>) -> WorldPacket {
...
pub(crate) fn read_cemetery_list_response(bytes: &[u8]) -> (bool, Vec<u32>) {
...
pub(crate) fn auto_guild_bank_item_packet(
    banker: ObjectGuid,
    bank_tab: u8,
    bank_slot: u8,
...
pub(crate) fn guild_bank_activate_packet(banker: ObjectGuid, full_update: bool) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_guid(&banker);
...
pub(crate) fn guild_bank_query_tab_packet(
    banker: ObjectGuid,
    tab: u8,
    full_update: bool,
) -> WorldPacket {
...
pub(crate) fn guild_bank_money_packet(banker: ObjectGuid, money: u64) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_guid(&banker);
...
pub(crate) fn guild_bank_buy_tab_packet(banker: ObjectGuid, tab: u8) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_guid(&banker);
    pkt.write_uint8(tab);
    pkt.reset_read();
...
pub(crate) fn guild_bank_update_tab_packet(
    banker: ObjectGuid,
    tab: u8,
    name: &str,
    icon: &str,
...
pub(crate) fn guild_bank_tab_query_packet(tab: i32) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_int32(tab);
    pkt.reset_read();
    pkt
...
pub(crate) fn guild_bank_set_tab_text_packet(tab: i32, text: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_int32(tab);
...
pub(crate) fn auto_store_guild_bank_item_packet(
    banker: ObjectGuid,
    bank_tab: u8,
    bank_slot: u8,
) -> WorldPacket {
...
pub(crate) fn battlemaster_hello_packet(unit: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn battlefield_list_packet(list_id: i32) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn battlemaster_join_packet(
    queue_ids: &[u64],
    roles: u8,
    blacklist_map: [i32; 2],
) -> WorldPacket {
...
pub(crate) fn battlemaster_join_arena_packet(team_size_index: u8, roles: u8) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn battlemaster_join_skirmish_packet(
    bg_type_id: u32,
    bracket_id: u32,
    as_group: u8,
    is_rated: u8,
...
pub(crate) fn battlefield_port_packet(
...
pub(crate) fn resurrect_response_packet(resurrecter: ObjectGuid, response: u32) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn repop_request_packet(check_instance: bool) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn port_graveyard_packet() -> WorldPacket {
    WorldPacket::new_empty()
}

pub(crate) fn reclaim_corpse_packet(corpse_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_bytes(&corpse_guid.to_raw_bytes());
    pkt
}

pub(crate) fn update_account_data_packet(
...
pub(crate) fn request_account_data_packet(player_guid: ObjectGuid, data_type: u8) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_packed_guid(&player_guid);
    pkt.write_bits(u32::from(data_type), 4);
    pkt.flush_bits();
...
pub(crate) fn activate_taxi_packet(
    vendor: ObjectGuid,
    node: u32,
    ground_mount_id: u32,
    flying_mount_id: u32,
...
pub(crate) fn bug_report_packet(report_type: bool, diag_info: &str, text: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn submit_user_feedback_packet(is_suggestion: bool, note: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_int32(571);
...
pub(crate) fn support_ticket_submit_bug_packet(message: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_int32(571);
    pkt.write_float(1.25);
    pkt.write_float(2.5);
...
pub(crate) fn support_ticket_submit_complaint_packet(note: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn support_ticket_submit_suggestion_packet(message: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_bits(message.len() as u32, 10);
    pkt.write_string(message);
    pkt
...
pub(crate) fn object_update_recovery_packet(guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn stand_state_change_packet(state: u32) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn can_duel_packet(target_guid: ObjectGuid, to_the_death: bool) -> WorldPacket {
    let mut packet = WorldPacket::new_empty();
...
pub(crate) fn duel_response_packet(
...
pub(crate) fn accept_trade_packet(state_index: u32) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn clear_trade_item_packet(trade_slot: u8) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn set_trade_item_packet(
    trade_slot: u8,
    pack_slot: u8,
...
pub(crate) fn set_trade_gold_packet(coinage: u64) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn set_trade_spell_packet(
    spell_id: u32,
    pack_slot: u8,
    item_slot_in_pack: u8,
) -> WorldPacket {
...
pub(crate) fn sign_petition_packet(petition_guid: ObjectGuid, choice: u8) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn decline_petition_packet(petition_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_bytes(&petition_guid.to_raw_bytes());
    pkt.reset_read();
    pkt
...
pub(crate) fn query_petition_packet(petition_id: u32, item_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn save_cuf_profiles_packet(
...
pub(crate) fn collection_item_set_favorite_packet(
    collection_type: u32,
    id: u32,
    favorite: bool,
) -> WorldPacket {
...
pub(crate) fn battle_pet_clear_fanfare_packet(pet_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn battle_pet_delete_pet_packet(pet_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn cage_battle_pet_packet(pet_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_uint16(0xBADD);
    pkt.write_packed_guid(&pet_guid);
    pkt
...
pub(crate) fn battle_pet_modify_name_packet(
    pet_guid: ObjectGuid,
    name: &str,
...
pub(crate) fn battle_pet_set_flags_packet(
    pet_guid: ObjectGuid,
    flags: u16,
    control_type: u8,
) -> WorldPacket {
...
pub(crate) fn battle_pet_set_battle_slot_packet(pet_guid: ObjectGuid, slot: u8) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn battle_pet_summon_packet(pet_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn battle_pet_update_notify_packet(pet_guid: ObjectGuid) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
...
pub(crate) fn query_battle_pet_name_packet(
...
pub(crate) fn battle_pet_request_journal_lock_packet() -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_uint16(ClientOpcodes::BattlePetRequestJournalLock as u16);
    pkt
}

pub(crate) fn battle_pet_request_journal_packet() -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
    pkt.write_uint16(ClientOpcodes::BattlePetRequestJournal as u16);
    pkt
}

pub(crate) fn accept_wargame_invite_packet(inviter_name: &str) -> WorldPacket {
    let mut pkt = WorldPacket::new_empty();
