...
impl<S: std::borrow::Borrow<PlayerReputationStateLikeCpp>> ReputationMgrLikeCpp<S> {
    /// C++ `ReputationMgr::GetStateList`, in `ReputationListID` order.
    pub fn factions(&self) -> impl Iterator<Item = &FactionStateLikeCpp> {
        self.state().factions_like_cpp()
    }
    /// C++ `ReputationMgr::GetForcedReactions`.
    pub fn forced_reactions(&self) -> impl Iterator<Item = (u32, ReputationRankLikeCpp)> + '_ {
        self.state().forced_reactions_like_cpp()
    }
    pub fn rank_counters(&self) -> ReputationRankCountersLikeCpp {
        self.state().rank_counters_like_cpp()
    }
...
impl<S: std::borrow::BorrowMut<PlayerReputationStateLikeCpp>> ReputationMgrLikeCpp<S> {
    pub fn get_state_mut(
        ...
        self.state_mut().faction_mut_like_cpp(rep_list_id)
    }
    pub fn initialize_like_cpp(
        ...
        for faction_entry in faction_store.iter() {
            ...
            self.state_mut().insert_faction_like_cpp(state);
        }
    }
    ...
    pub fn apply_force_reaction_like_cpp(
        ...
        if apply {
            self.state_mut()
                .set_forced_reaction_like_cpp(faction_id, Some(rank));
        } else {
            self.state_mut()
                .set_forced_reaction_like_cpp(faction_id, None);
        }
    }
    pub fn set_reputation_like_cpp(
        ...
        if self.state().contains_faction_like_cpp(rep_list_id) {
    ...
    pub fn load_from_db_like_cpp(
        ...
        for row in rows {
            ...
            if let Some(faction) = self.state_mut().faction_mut_like_cpp(rep_list_id) {
    ...
    pub fn set_at_war_by_replist_like_cpp(
        ...
        let Some(faction_id) = self
            .state()
            .faction_like_cpp(rep_list_id)
            .map(|state| state.faction_id)
        else {
            return false;
        };
        ...
        before.as_ref() != self.state().faction_like_cpp(rep_list_id)
    }
