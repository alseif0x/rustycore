# Hallazgos de contraste C++ — registro histórico acotado

Origen: pasadas iniciadas el 2026-06-27, con correcciones incrementales anotadas.
Edición documental: 2026-09-05; **no se volvió a auditar producción ni se
certificó el conjunto del port**. Se conservan hallazgos que ya citaban C++,
sus límites, pendientes e identificadores históricos para trazabilidad.

Los veredictos describen el Rust observado en aquellas pasadas: un ancla o
una fila marcada como corregida no prueba el HEAD actual. Sólo las ramas
realmente contrastadas constituyen evidencia histórica; los pendientes son
pistas, no bugs confirmados por este cambio documental.

Las etiquetas `#LegacyAudit.<sufijo>` son alias exclusivamente documentales de
los identificadores históricos recuperables en Git `7eaf8ddc`; conservan sus
sufijos y relaciones. No representan pruebas nuevas ni IDs de ejecución. Los
nombres de tests, el código, los TSV y las issues no se renombran.

Estado vigente: [STATE.md](../migration/STATE.md), la issue/checkpoint y
[EXISTING-CODE-DEFECTS.md](../migration/EXISTING-CODE-DEFECTS.md).
Navegación: [índice C++](cpp-parity-audit-index.md) y
[checklist histórico](cpp-parity-checklist.md).
La única fuente de comportamiento es `/home/server/woltk-trinity-legacy`,
con capturas reales identificadas si el C++ es incompleto o ambiguo.

## Veredictos verificados

| Area | Rust ref | C++ ref | Veredicto | Evidencia |
| --- | --- | --- | --- | --- |
| ByteBuffer bits | `crates/wow-packet/src/world_packet.rs:391,410,419` | `src/server/shared/Packets/ByteBuffer.h:155-217`; `ByteBuffer.cpp:78-91` | No bug de comportamiento | `ResetBitPos`, `ReadBit`, `ReadBits`, `WriteBit` y `WriteBits` tienen equivalente C++. |
| Packet compression threshold | `crates/wow-packet/src/compression.rs:20-30`; `crates/wow-network/src/world_socket.rs:661-664,1194-1198,1333-1336`; test `compression_threshold_uses_payload_len_like_cpp` | `src/server/game/Server/WorldSocket.cpp:541-550`; `WorldSocket.cpp:582-609` | Bug corregido `#LegacyAudit.COMPRESS.1` | C++ comprime si `packet.size() > 0x400`, donde `packet.size()` es payload sin opcode. Rust ya resta el opcode antes de decidir, usa `COMPRESSION_THRESHOLD = 0x400` y el test cubre payload `0x3FF`, `0x400` y `0x401`. |
...
| FeatureSystemStatus config | `crates/wow-packet/src/packets/misc.rs:2040-2204`; `crates/wow-world/src/handlers/character.rs:13453`; `crates/wow-world/src/session.rs:21365-21382`; `crates/world-server/src/main.rs:4595-4639,11639-11658`; tests `feature_system_status_uses_cpp_config_flags` | `Handlers/CharacterHandler.cpp:1457-1485`; `World.cpp:584-588`; `World.cpp:1597-1599` | Bug corregido `#LegacyAudit.FEATURE.1` | C++ rellena support tickets/bugs/complaints/suggestions, `CharUndelete`, `BpayStore` e `IsMuted = !CanSpeak()` desde config/sesion. Rust ahora propaga esos configs a `WorldSession`, construye `FeatureSystemStatus::from_config_like_cpp` y el test verifica los bits C++ de BPay, undelete, muted y EuropaTicket. |
...
| SRP verifier almacenado | `bnet_srp6.rs:115-128` | `BigNumber.h:37,46`; `BigNumber.cpp:56`; `SRP6.cpp:36-38` | No bug | Rust lee `verifier` little-endian; C++ `BigNumber(std::vector<uint8>)` usa little-endian por defecto. |
...
| SRP v1 iterations | `crates/wow-crypto/src/bnet_srp6.rs:72,154-157`; test `challenge_v1_iterations_match_cpp` | `SRP6.h:165-166`; `LoginRESTService.cpp:468-470` | Bug corregido `#LegacyAudit.BNETSRP.1` | C++ `BnetSRP6v1Base::GetXIterations()` devuelve `1` y `LoginRESTService` serializa ese valor. Rust ahora devuelve `iterations=1` para v1 y conserva `15000` para v2. |
| SRP `u = H(A || B)` padding | `crates/wow-crypto/src/bnet_srp6.rs:189,419-445`; tests `compute_u_v1_pads_a_and_b_to_128_bytes_like_cpp`, `compute_u_v2_pads_a_and_b_to_256_bytes_like_cpp` | `SRP6.h:203-205`; `SRP6.h:226-228`; `BigNumber.h:127-133`; `BigNumber.cpp:189-193` | Bug corregido `#LegacyAudit.BNETSRP.2` | C++ hashea `A.ToByteArray<128>(false) || B.ToByteArray<128>(false)` en v1 y `A.ToByteArray<256>(false) || B.ToByteArray<256>(false)` en v2. Rust ahora paddea ambos operandos a ancho fijo antes del hash; los tests cubren ceros iniciales y prueban que el hash no usa bytes minimos. |
| SRP `k` v2+SHA512 | `crates/wow-crypto/src/bnet_srp6.rs:329-336,408-414`; test `compute_k_v2_sha512_pads_generator_to_256_bytes_like_cpp` | `SRP6.h:196`; `SRP6.h:218-219`; `BigNumber.h:127-133`; `BigNumber.cpp:189-193` | Bug corregido `#LegacyAudit.BNETSRP.3` | C++ calcula `k` con `N.ToByteArray<128/256>(false) || g.ToByteArray<128/256>(false)` para cualquier hash. Rust ahora usa el mismo ancho fijo tambien para v2+SHA512; el test demuestra que no usa `SHA512(N || g)` con bytes minimos. |
| SRP username normalizacion | `crates/wow-crypto/src/bnet_srp6.rs:271-284`; `crates/bnet-server/src/rest/handlers.rs:359-366,394-420,469-507`; tests `utf8_to_upper_only_latin_matches_cpp_basic_latin_only`, `srp_username_does_not_apply_unicode_uppercase_like_cpp` | `LoginRESTService.cpp:273-309`; `LoginRESTService.cpp:429-456`; `Util.h:124-130`; `Util.h:253-282`; `Util.cpp:795-803` | Bug corregido `#LegacyAudit.BNETSRP.4` | C++ aplica `Utf8ToUpperOnlyLatin`: solo `a..z` pasan a `A..Z`; caracteres no ASCII como `é`/`ß` quedan intactos. Rust ahora usa el mismo helper para DB lookup, SRP username y password v1; `srp_username` tambien normaliza internamente para no depender del caller. |
| SRP challenge cuenta inexistente | `crates/bnet-server/src/rest/handlers.rs:482-535`; test `srp_challenge_missing_account_returns_done_like_cpp` | `LoginRESTService.cpp:443-449` | Bug corregido `#LegacyAudit.BNETREST.3` | C++ responde JSON `authentication_state=DONE` con `Content-Type: application/json;charset=utf-8`, sin revelar si la cuenta existe. Rust ya devuelve el mismo shape/status mediante `srp_challenge_missing_account_response_like_cpp`. |
| SRP challenge hex casing | `crates/bnet-server/src/rest/handlers.rs:320,512-517,874-879`; test `hex_encode_uses_cpp_uppercase` | `LoginRESTService.cpp:320`; `LoginRESTService.cpp:471-488`; `Util.cpp:849-866` | Bug corregido `#LegacyAudit.BNETREST.4` | C++ usa `AsHexStr()`/`ByteArrayToHexStr()` uppercase para `server_evidence_m2`, `modulus`, `generator`, `salt` y `public_b`. Rust ahora usa uppercase en el helper comun `hex_encode`; el test cubre padding de nibble y letras A-F. |
| Login ticket hex casing | `crates/bnet-server/src/rest/handlers.rs:737-741,874-879`; test `make_login_ticket_uses_cpp_uppercase_hex` | `LoginRESTService.cpp:387-388`; `LoginRESTService.cpp:780`; `Util.cpp:849-866` | Bug corregido `#LegacyAudit.BNETREST.4` | C++ genera `TC-` + 20 bytes uppercase con `ByteArrayToHexStr`. Rust ahora genera el ticket con el mismo helper uppercase y el test valida longitud 40, prefijo `TC-` y ausencia de `a..f`. |
| Login directo error invalido | `handlers.rs:383`; `handlers.rs:440-449`; `handlers.rs:882-891` | `LoginRESTService.cpp:279-289`; `LoginRESTService.cpp:374-383` | No bug | Rust termina usando `error_result()` con `DONE` y campos nulos, igual que C++. |
...
Contraste de `crates/wow-packet/src/packets/spell.rs`
...
| `CastFailed` / `SpellCastVisual` | `spell.rs:196-214`; `spell.rs:758-780`; tests `spell_cast_visual_serializes_one_int32_like_cpp`, `cast_failed_writes_visual_between_spell_and_reason_like_cpp` | `SpellPackets.h:449-461`; `SpellPackets.cpp:514-523`; `CombatLogPacketsCommon.h:86-90`; `CombatLogPacketsCommon.cpp:164-176` | Bug corregido `#LegacyAudit.SPELL.1` | C++ escribe `CastID`, `SpellID`, `SpellCastVisual`, `Reason`, `FailedArg1`, `FailedArg2`, y `SpellCastVisual` serializa solo `SpellXSpellVisualID` porque `ScriptVisualID` esta comentado. Rust ahora escribe el visual en ese punto y el helper compartido ocupa un solo `int32`, evitando desplazar `Reason`/argumentos y el resto de paquetes spell que usan el helper. |
...
Contraste de `crates/wow-packet/src/packets/combat.rs`.
...
| Chat listen ranges | `world-server/src/main.rs`; `wow-network/src/accept.rs`; `wow-world/src/session.rs`; `wow-world/src/handlers/chat.rs`; tests `chat_listen_ranges_use_cpp_world_config_keys`, `say_uses_cpp_configured_listen_range_like_cpp`, `chat_emote_uses_cpp_configured_text_emote_range_like_cpp`, `send_text_emote_uses_cpp_configured_text_emote_range_like_cpp`, `yell_uses_cpp_configured_listen_range_like_cpp` | `World.cpp:1323-1325`; `worldserver.conf.dist:1906-1926`; `ChatHandler.cpp:247-292,713-719`; `Player.cpp:21073-21114` | Bug corregido `#LegacyAudit.CHAT.1` | C++ carga `CONFIG_LISTEN_RANGE_SAY`, `CONFIG_LISTEN_RANGE_TEXTEMOTE` y `CONFIG_LISTEN_RANGE_YELL` desde `ListenRange.*` con fallback de codigo 25/25/300, aunque la config distribuida use Say/TextEmote 40. Rust ahora lee los mismos keys, los propaga a `WorldSession` y los usa para say, yell, `/e` y `CMSG_SEND_TEXT_EMOTE`. |
...
Contraste de `crates/wow-packet/src/packets/party.rs`
...
| `PartyInviteServer` valores | `group.rs:840`; `group.rs:947-959`; `party.rs:632-673`; test `party_invite_server_uses_cpp_inviter_values_like_cpp` | `PartyPackets.cpp:62-98`; `PartyPackets.h:60-82`; `GroupHandler.cpp:181` | Bug corregido `#LegacyAudit.PARTY.1` | C++ propaga `packet.ProposedRoles`, `InviterBNetAccountId = session account GUID`, y realm actual/normalizado; `AllowMultipleRoles` queda false por defecto. Rust ahora alimenta esos valores desde `WorldSession` y escribe `ProposedRoles` en vez de cero. |
| `HandlePartyInviteOpcode` validaciones | `group.rs:819-1010` | `GroupHandler.cpp:58-184`; `SocialMgr.cpp:184-191`; `World.cpp:790,913,1163`; `SharedDefines.h:6787-6803` | Bug corregido `#LegacyAudit.PARTY.2` | Rust ya sigue el orden C++ de invite: target/self, `GM.AllowInvite`, `AllowTwoSide.Interaction.Group`, instancia/difficulty, `HasIgnore`, `HasFriend` + `PartyLevelReq`, target grouped/invited, permisos leader/assistant, `Group::IsFull()` 5/40 y OK. `HasIgnore` cubre GUID y cuenta del inviter mediante `character_social` unido a `characters.account`. |
| PartyIndex invite/accept/leave | `group.rs:819-1118`; `group.rs:1293-1430`; tests `party_invite_party_index_instance_does_not_use_full_home_group_like_cpp`, `party_invite_response_party_index_mismatch_keeps_invite_pending_like_cpp`, `leave_group_party_index_instance_does_not_leave_home_group_like_cpp` | `GroupHandler.cpp:114-120`; `GroupHandler.cpp:189-195`; `GroupHandler.cpp:335-357`; `Player.cpp:23447-23460` | Bug corregido `#LegacyAudit.PARTY.3` | Rust ahora conserva `PartyIndex` en invite/accept/leave y lo pasa al lookup representado equivalente a `Player::GetGroup(packet.PartyIndex)`. `PartyIndex=INSTANCE` ya no cae al grupo HOME para full-check, accept ni leave; el invite pendiente se conserva si la categoria aceptada no coincide. |
| `HandlePartyInviteResponseOpcode` lifecycle | `group.rs:1196-1435` | `GroupHandler.cpp:187-238`; `Group.cpp:336-390,406-486`; `Player.cpp:2145-2165` | Bug corregido `#LegacyAudit.PARTY.4` | Rust representa `Player::m_groupInvite` con `PendingInviteLikeCpp`, conserva invite ante `PartyIndex` mismatch, borra invite antes de self/full como C++, revalida `Group::IsFull()`, crea grupo pendiente solo al primer accept con lider online, promueve invitees al grupo creado y aplica `Player::SetGroup` remoto al lider. |
| `HandleLeaveGroupOpcode` lifecycle | `group.rs:1662-1846` | `GroupHandler.cpp:333-357`; `Group.cpp:550-785`; `Player.cpp:2145-2165` | Bug corregido `#LegacyAudit.PARTY.4` | Rust ya resuelve grupo real o `GroupInvite`, rechaza battleground representado con `ERR_INVITE_RESTRICTED`, envia `PARTY_OP_LEAVE` OK antes de remover, y si solo existe pending invite del lider cancela todas las invitees del grupo pendiente como `grpInvite->Disband()`. |
...
| `InvUpdate` | `item.rs:354-376` | `ItemPacketsCommon.cpp:247-258`; `ItemPacketsCommon.h:106-117` | No bug de parser | C++ lee count de 2 bits, reset bit pos, y pares `ContainerSlot`, `Slot`. Rust hace lo mismo. |
...
| `AutoStoreBagItem` parser | `item.rs:486-513`; test `auto_store_bag_item_parses` | `ItemPackets.cpp:223-229`; `ItemPackets.h:290-300`; `ItemHandler.cpp:707-734` | Bug corregido `#LegacyAudit.ITEM.1` | C++ lee `Inv`, `ContainerSlotB`, `ContainerSlotA`, `SlotA`; despues usa `ContainerSlotA` como source y `ContainerSlotB` como destino. Rust ahora lee en ese orden y el test usa valores distintos para A/B para cubrir que no se invierten. |
...
| Inventory slot constants | `character.rs:12235-12241`; `wow-entities/src/player.rs:760-765` | `Player.h:621`; `Player.h:671-684` | No bug de constantes base | `INVENTORY_SLOT_BAG_0=255`, bag slots `30..34` e item slots `35..59` coinciden con C++. |
| `HandleSwapInvItemOpcode` runtime | `character.rs:11604-11753` | `ItemHandler.cpp:69-112`; `Player.cpp:12295-12530` | Bug runtime / parcial | C++ exige `Inv.Items.size()==2`, valida `IsValidPos`, bank access, y delega en `Player::SwapItem`. Rust ya corta por count invalido; siguen pendientes checks de bank/posicion y la logica completa de `SwapItem`. |
| `HandleAutoEquipItemOpcode` runtime | `character.rs:11755-11830` | `ItemHandler.cpp:175-291` | Bug runtime / parcial | C++ exige `Inv.Items.size()==1`, usa `CanEquipItem`, `CanUnequipItem`, `CanStoreItem`/`CanBankItem`, mueve items con `RemoveItem`/`EquipItem`/`StoreItem`, auto-unequip offhand y auras dependientes. Rust ya corta por count invalido, pero aun calcula un slot por `InventoryType` y delega a un swap simplificado. |
...
| `HandleSwapItem` / `HandleAutoStoreBagItemOpcode` runtime | `character.rs:11891-11976` | `ItemHandler.cpp:130-173`; `ItemHandler.cpp:699-743` | Bug runtime / parcial | C++ soporta posiciones container-aware, valida `Inv.Items` esperado, `IsValidPos`, bank access, `CanStoreItem` y `StoreItem`. Rust ya corta por counts invalidos (`SwapItem=2`, `AutoStoreBagItem=0`), pero solo soporta `container=255`, envia `InternalBagError` para nested bags y busca un backpack slot libre por cuenta propia. |
...
Contraste parcial de `crates/wow-packet/src/packets/misc.rs`
...
| Area | Rust ref | C++ ref | Veredicto | Evidencia |
| --- | --- | --- | --- | --- |
| Dungeon/Raid difficulty packets | `misc.rs:4534-4567` | `MiscPackets.cpp:250-263`; `Player.cpp:20669-20683`; `CharacterHandler.cpp:1113` | No bug de layout | C++ escribe `int32 DifficultyID` y raid anade `uint8 Legacy`. Rust coincide. |
...
| `SetProficiency::default_*` masks | `misc.rs:4831-4883`; `session.rs:14084-14090` | `SpellEffects.cpp:1785-1805`; `Player.cpp:244-245`; `Player.cpp:11093-11143`; `Player.cpp:21780-21787` | Bug confirmado / tabla no canonica | El packet layout es correcto, y `EffectProficiency` Rust envia masks acumulados como C++. Pero C++ no tiene tabla fija de proficiencies por clase: inicia `m_WeaponProficiency/m_ArmorProficiency=0`, OR-ea masks por spells, y `CanUseItem` valida required skill/spell. Rust usa `default_weapons(class)` como tabla fija en un gate de item visual/collection. Ver pasada 12. |
...
| `ShowTradeSkill` request/response | `misc.rs:ShowTradeSkill`; `character.rs:handle_show_trade_skill`; `session.rs:ShowTradeSkill` | `Packet.h:71-77`; `MiscHandler.cpp:1416-1419`; `Opcodes.cpp:925`; `Opcodes.cpp:2095` | Bug corregido `#LegacyAudit.MISC.1` | C++ registra `CMSG_SHOW_TRADE_SKILL` con `WorldPackets::Null`, cuyo `Read()` hace `rfinish()`, y el handler solo loguea; `SMSG_SHOW_TRADE_SKILL_RESPONSE` figura `STATUS_UNHANDLED`. Rust ahora consume el CMSG como null/no-op y no envia SMSG. |
...
| `AuctionHelloResponse` | `misc.rs:6007-6046`; test `auction_hello_response_writes_cpp_layout_without_auction_house_id` | `AuctionHousePackets.cpp:622-631`; `AuctionHousePackets.h:474-485`; `AuctionHouseHandler.cpp:995-1010` | Bug corregido `#LegacyAudit.MISC.2` | C++ escribe `Guid`, `PurchasedItemDeliveryDelay`, `CancelledItemDeliveryDelay`, bit `OpenForBusiness`; no serializa `AuctionHouseID`. `SendAuctionHello` no setea delays, por defecto quedan `0`. Rust ahora omite `AuctionHouseID` y `open()` deja ambos delays en cero. |
...
Contraste de rutas explicitas en
...
| Character flags from DB | `character.rs:2535-2564`; `character.rs:2585-2599` | `CharacterPackets.cpp:119-148`; `SharedDefines.h:1035-1036` | Bug confirmado | C++ limpia ghost si `AT_LOGIN_RESURRECT`, mapea ghost y rename, anade `LOCKED_BY_BILLING` si hay ban, `DECLINED` si config/declined name, y muestra pet family/display solo para clases permitidas y no ghost. Rust ademas mapea `PLAYER_FLAGS_RESTING` a `CHARACTER_FLAG_RESTING`, pero C++ no lo hace en esta ruta; tambien lee `_banned_guid` pero no setea locked billing, no setea declined flag y deja `pet_family=0`. |
...
Esta pasada trata `crates/wow-world/src/handlers/loot.rs` como subsistema,
no como solo el comentario de `DoLootRelease`. Alcance cerrado aqui: packet layouts basicos, apertura de
...
| `LootResponse` valores success/error | `loot.rs:23-24`; `handlers/loot.rs:370-382`; `500-512`; `1052-1064`; `3926-3938`; `6320-6332` | `LootPackets.h:67-72`; `LootPackets.cpp:38-45`; `Player.cpp:8758-8765`; `Player.cpp:8778-8783`; `Loot.h:136-151` | Corregido | C++ success deja `FailureReason=17` y `Threshold=2` por default, luego setea `Acquired=true`; C++ error solo pisa `FailureReason` y conserva `Threshold=2`. Rust ahora usa `LOOT_RESPONSE_DEFAULT_FAILURE_REASON_LIKE_CPP`/`LOOT_RESPONSE_DEFAULT_THRESHOLD_LIKE_CPP` en los success y conserva threshold C++ en `send_loot_error_like_cpp`. Tests: `loot_response_success_defaults_write_cpp_failure_reason_and_threshold`, `loot_response_success_keeps_cpp_failure_and_threshold_defaults`, `loot_error_response_keeps_cpp_threshold_default_like_cpp`. |
...
por clase de `SetProficiency::default_weapons/default_armor` tenian un equivalente en C++.
...
| `CMSG_REQUEST_CEMETERY_LIST` | `handlers/misc.rs:2395-2499`; `session.rs`; `world-server/src/main.rs` | `MiscPackets.h:391-397`; `MiscHandler.cpp:367-399` | Bug corregido `#LegacyAudit.CEMETERY.1` | C++ request no tiene campos leidos por el handler, busca graveyards por `zoneId`, filtra conditions y envia hasta 16 IDs; si no hay IDs, no envia respuesta. Rust ya no consume byte adicional sin equivalente canónico, recibe `GraveyardStore` cargado con condiciones, responde con IDs de la zona y mantiene `IsGossipTriggered=false`. |
...
El contraste de Quest abarca `wow-packet`, `wow-data` y `wow-world`.
...
| Area | Rust ref | C++ ref | Veredicto | Evidencia |
| --- | --- | --- | --- | --- |
| Constantes Quest counts | `packets/quest.rs:15-19`; `wow-data/src/quest.rs:16-21` | `QuestDef.h:45-53`; `QuestPackets.h:190-200`; `QuestPackets.h:272-285` | No bug de valores | Los counts Rust `items=4`, `choice=6`, `reputation=5`, `currency=4`, `display_spells=3`, `item_drop=4` coinciden con C++ `QUEST_*_COUNT`. |
...
| `QuestXP::calculate_xp` fila ausente | `quest_xp.rs:71-112`; test `calculate_xp_missing_quest_level_returns_zero_like_cpp` | `QuestDef.cpp:387-398` | Bug corregido `#LegacyAudit.QUESTXP.1` | C++ hace `sQuestXPStore.LookupEntry(quest_level)` y devuelve `0` si no existe fila. Rust ahora exige la fila exacta y no usa nivel cercano. |
| `QuestXP::calculate_xp` min scaled XP | `quest_xp.rs:103-113`; `session.rs:25228-25234`; `main.rs:4576-4580`; test `calculate_xp_min_scaled_ratio_raises_grey_quest_like_cpp` | `QuestDef.cpp:404-409`; world config `CONFIG_MIN_QUEST_SCALED_XP_RATIO` | Bug corregido `#LegacyAudit.QUESTXP.2` | C++ aplica `max(minScaledXP, xp)` si la config `CONFIG_MIN_QUEST_SCALED_XP_RATIO` es distinta de cero. Rust ahora transporta esa config a `WorldSession` y la aplica al calculo DB2. |
| `QuestTemplate::is_available_for` | `wow-data/src/quest.rs:279-301` | `Player.cpp:15060-15089`; `Player.cpp:15240-15260`; `Player.cpp:15037-15059` | No bug en race/class/min/max | Rust comprueba mask de raza, mask de clase, min level y max level igual que las gates C++ `SatisfyQuestRace`, `SatisfyQuestClass` y `SatisfyQuestLevel`. |
| `CMSG_QUEST_GIVER_QUERY_QUEST` / `ACCEPT_QUEST` bit flags | `handlers/quest.rs:131-148`; `handlers/quest.rs:1361-1394` | `QuestPackets.cpp:547-557`; `QuestHandler.cpp:228-253`; `QuestHandler.cpp:105-225` | Bug corregido `#LegacyAudit.QUESTCMSG.1` | C++ lee `RespondToGiver`/`StartCheat` con `ReadBit()` despues de GUID y QuestID. Rust ya usa `read_bit()` equivalente y cubre `0x01` como false, no como bool-byte true. |
...
Los comentarios de `wow-data` no prueban comportamiento. Esta pasada
...
| Fuente y proyeccion de stats base | `player_stats.rs::{PlayerStatsStore,calculate_player_stat_system_like_cpp}`; `game_tables.rs::BaseMpGameTableLikeCpp`; `character.rs::player_stat_changes_with_represented_item_bonuses_like_cpp` | `ObjectMgr.cpp:4140-4247`; `ObjectMgr.cpp:4411-4427`; `Player.cpp:2365-2420`; `StatSystem.cpp:106-209`; `StatSystem.cpp:279-425`; `StatSystem.cpp:502-731`; `SpellAuraEffects.cpp::HandleModTotalPercentStat` | Corregido en issue #60 / PR #118 | Rust elimina `player_levelstats` de esta ruta: carga `player_racestats` + `player_classlevelstats`, completa niveles ausentes como C++, toma mana base de `gt/BaseMp.txt`, conserva `CreateHealth=0` y usa una proyeccion compartida para create/login/equipo/level-up con health/power/armor/AP/ratings/crit/dodge/parry/spell-crit. El aura pasiva `SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE` usa `MiscValueB` como mascara y recalcula al aplicar/retirar. Captura acreditada C++/Rust del mismo paladin humano nivel 80: max health `1460`, max mana `6100`, stats `[178,92,164,100,111]`, armor `184`, base mana `4880`, AP `576` y damage `83.285713/84.285713` identicos. El `UpdateObject` completo conserva divergencias ajenas a stats; el resto de unit mods/auras y la aplicacion amplia de stats de items permanecen en sus ledgers propios. |
| `ItemModType` subset | `item_stats.rs:22-52` | `ItemTemplate.h:28-100` | No bug de valores para el subset listado | Los valores Rust visibles para mana, health, primarios, ratings unificados, AP/RAP, expertise, armor penetration y spell power coinciden con el enum C++ en ese subset. |
...
| `SkillLineAbility` DB2 layout e identidad usados | `skill.rs::SkillStore::load`; `wdc4.rs::iter_records` | `DB2Structure.h:3324-3341`; `DB2LoadInfo.h:4546-4569`; `DB2Metadata.h::SkillLineAbilityMeta` | Corregido en issue #62 | El field 1 es el `ID` C++ dentro de `SkillLineAbilityLoadInfo` y tambien el `id_index=1` del metadata WDC4. Rust antes fabricaba `min_id + index` cuando no habia lista externa, rompiendo los campos Common keyed por record ID. `iter_records()` usa ahora el ID inline real y los campos `RaceMask`, `SkillLine`, `Spell`, ranks, masks, flags y `SkillupSkillLineID` quedan alineados. |
...
| `starting_skills` como player-create/login skills | `skill.rs::SkillStore::load`; `default_starting_skill_info_like_cpp`; `character.rs` login skill load | `ObjectMgr.cpp:3982-3996`; `Player.cpp:23822-23838` | Corregido en issue #62 | Rust solo indexa `SkillRaceClassInfo` con `Availability == 1`, salta `MinLevel > level`, conserva skills sin abilities y fusiona las filas persistidas antes de aprender defaults ausentes. |
| Valores de default/loaded skill | `skill.rs::default_skill_info_like_cpp`; `loaded_skill_info_like_cpp`; `character.rs` SkillInfo login path | `Player.cpp:23840-23879`; `Player.cpp::_LoadSkills`; `ObjectMgr.h:957-974`; `DBCEnums.h:1869-1876` | Corregido en issue #62 | Language `300/300`, level `1/max` o always-max, DK `(level-1)*5`, mono `1/1`, rank con tier/step y normalizacion de filas persistidas siguen la rama C++. La informacion canonica alimenta `SkillInfo` y el valor real usado para recompensas. |
| `LearnSkillRewardedSpells` en login live | `skill.rs::skill_rewarded_spell_changes_like_cpp`; `character.rs` no-DB-spell login path | `Player.cpp:23954-24006`; `DB2Stores.cpp:2578-2580`; `DBCEnums.h:1862-1865` | Corregido en issue #62 | La ruta live usa niveles reales de `SpellInfo`, acquire 1/2 y fallback quest con flag/condition, Riding con `NumSkillUps == 1`, race/class masks, required level, `MinSkillLineRank`, remove-below-rank y el valor real de cada skill. Un login Hunter Blood Elf produce el mismo set exacto de 43 spells que C++; el comparador solo canonicaliza el orden de `unordered_map`. |

## Pasada 16: subsistema Movement packets/handlers

El contraste de Movement abarca parser binario, opcodes y handler runtime.
...
| Handler generico usa `GetUnitBeingMoved()` | `handlers/movement.rs:105-364`; test `handle_movement_uses_current_mover_guid_like_cpp` | `MovementHandler.cpp:312-430` | Bug corregido `#LegacyAudit.MOVEMENT.1` | C++ toma `Unit* mover = _player->GetUnitBeingMoved()`, valida `movementInfo.guid` contra ese mover, fuerza `movementInfo.guid = mover->GetGUID()`, guarda tiempo/flags/posicion en el mover y emite `MoveUpdate` desde ese GUID. Rust ya usa `player_moved_unit_guid_like_cpp()` como mover esperado, no pisa posicion/flags del player cuando el mover es otro unit, reubica la criatura representada si existe en el map manager y manda `SendIfVisibleLikeCpp` con `source_guid=mover_guid`. |
...
1. `#LegacyAudit.COMPRESS.1`: corregido. El threshold de compresion usa payload sin opcode como C++ (`packet.size() > 0x400`); test: `compression_threshold_uses_payload_len_like_cpp`.
2. `#LegacyAudit.BNETREST.1`: corregido. `GET /bnetserver/login/` ya no emite `JSESSIONID`; C++ solo setea `Content-Type` y body JSON. Test: `login_form_headers_do_not_set_cookie_like_cpp`.
3. `#LegacyAudit.BNETREST.2`: corregido. `POST /bnetserver/login/srp/` ya no emite ni depende de `JSESSIONID`; el SRP vive en `RestConnectionState` por conexion HTTP como `LoginHttpSession::GetSessionState()` en C++.
4. `#LegacyAudit.FEATURE.1`: corregido. `FeatureSystemStatus` y `FeatureSystemStatusGlueScreen` ya reflejan los configs C++ de support/BPay/undelete/max chars/expansion y `IsMuted = !CanSpeak()`. Tests: `feature_system_status_uses_cpp_config_flags`, `feature_system_status_glue_screen_uses_cpp_config_fields`.
5. `#LegacyAudit.BNETSRP.1`: corregido. Challenge v1 envia `iterations=1` como C++ `BnetSRP6v1Base::GetXIterations()`. Test: `challenge_v1_iterations_match_cpp`.
6. `#LegacyAudit.BNETSRP.2`: corregido. Calculo SRP `u` paddea `A`/`B` a 128 bytes en v1 y 256 bytes en v2 como C++ `CalculateU`. Tests: `compute_u_v1_pads_a_and_b_to_128_bytes_like_cpp`, `compute_u_v2_pads_a_and_b_to_256_bytes_like_cpp`.
7. `#LegacyAudit.BNETSRP.3`: corregido. `k` para v2+SHA512 paddea `g` a 256 bytes como C++ `BnetSRP6v2`. Test: `compute_k_v2_sha512_pads_generator_to_256_bytes_like_cpp`.
8. `#LegacyAudit.BNETSRP.4`: corregido. Login/SRP username y password v1 usan `Utf8ToUpperOnlyLatin` como C++, sin uppercase Unicode completo. Tests: `utf8_to_upper_only_latin_matches_cpp_basic_latin_only`, `srp_username_does_not_apply_unicode_uppercase_like_cpp`.
9. `#LegacyAudit.BNETREST.3`: corregido. SRP challenge de cuenta inexistente devuelve JSON `authentication_state=DONE` como C++, sin HTTP 400 ni mensaje `Account not found`. Test: `srp_challenge_missing_account_returns_done_like_cpp`.
10. `#LegacyAudit.BNETREST.4`: corregido. Challenge REST, `server_evidence_m2` y login ticket usan hex uppercase como C++ `AsHexStr()`/`ByteArrayToHexStr()`. Tests: `hex_encode_uses_cpp_uppercase`, `make_login_ticket_uses_cpp_uppercase_hex`.
11. `#LegacyAudit.SPELL.1`: corregido; `SMSG_CAST_FAILED` serializa `SpellCastVisual` entre `SpellID` y `Reason`, y el helper `SpellCastVisual` ocupa un solo `int32` como C++.
12. `#LegacyAudit.COMBAT.1`: corregido. `SMSG_ATTACKER_STATE_UPDATE` escribe el bit de combat-log en el paquete exterior antes del size, y `attackRoundInfo` queda sin byte extra.
13. `#LegacyAudit.CHAT.1`: corregido. Rangos chat/emote/yell salen de `ListenRange.*` (`CONFIG_LISTEN_RANGE_*`) y se aplican a say, yell, `/e` y `CMSG_SEND_TEXT_EMOTE` como C++.
14. `#LegacyAudit.CHAT.2`: handlers `CMSG_EMOTE`/`CMSG_SEND_TEXT_EMOTE` son runtime simplificado y no reproducen validacion/scripts/criteria/DB2/rango C++.
15. `#LegacyAudit.PARTY.1`: corregido. `SMSG_PARTY_INVITE` usa roles propuestos, account GUID, realm actual/normalizado y `AllowMultipleRoles=false` como C++ `PartyInvite::Initialize`.
16. `#LegacyAudit.PARTY.2`: corregido. `HandlePartyInviteOpcode` usa los knobs C++ `GM.AllowInvite`, `AllowTwoSide.Interaction.Group` y `PartyLevelReq`, y aplica `HasIgnore`/`HasFriend` del target en el orden de `GroupHandler.cpp`.
17. `#LegacyAudit.PARTY.3`: corregido. Invite/accept/leave conservan `PartyIndex` y no operan sobre HOME cuando C++ resolveria otra categoria o rechazaria por mismatch.
18. `#LegacyAudit.PARTY.4`: `HandlePartyInviteResponseOpcode` y `HandleLeaveGroupOpcode` modelan un lifecycle distinto al C++ (`GetGroupInvite`, self/full/category checks, cancelacion de invite y `PartyResult` OK de leave).
19. `#LegacyAudit.ITEM.1`: corregido. `AutoStoreBagItem` lee `ContainerSlotB` antes de `ContainerSlotA` como C++ y conserva A=source/B=destino.
20. `#LegacyAudit.ITEM.2`: parcial. Handlers inventory move/equip/store ya aplican los gates C++ de `InvUpdate` para counts invalidos, pero siguen direct-inventory-only o simplificados y omiten validaciones/logica C++ de `IsValidPos`, bank access, `CanEquipItem`, `CanStoreItem`, `CanUnequipItem`, `DestroyItem` container-aware y `Player::SwapItem`.
21. `#LegacyAudit.MISC.1`: corregido. `ShowTradeSkill` consume el CMSG como `WorldPackets::Null`/`rfinish()` y no envia `SMSG_SHOW_TRADE_SKILL_RESPONSE`, igual que el handler C++ no-op.
22. `#LegacyAudit.MISC.2`: corregido. `AuctionHelloResponse` omite `AuctionHouseID` y deja delays default 0 como C++ `SendAuctionHello`.
23. `#LegacyAudit.CHARACTER.1`: enum character flags/data mapping diverge: Rust mapea resting, omite locked billing/declined y no rellena pet family como C++.
24. `#LegacyAudit.CHARACTER.2`: logout request completa logout instantaneo siempre; C++ puede denegar, responder delayed y arrancar countdown/root/stun.
25. `#LegacyAudit.LOOT.1`: corregido. `LootResponse` success conserva los defaults C++ `FailureReason=17` y `Threshold=2`; el error setea `FailureReason` especifico pero conserva `Threshold=2`.
26. `#LegacyAudit.LOOT.2`: apertura de loot no replica `Player::isAllowedToLoot`: falta `HasPendingBind` y gating por `LootMethod`/round-robin/over-threshold.
27. `#LegacyAudit.LOOT.3`: `HandleLootMoneyOpcode` no aplica `SPELL_AURA_MOD_MONEY_GAIN` ni criteria `MoneyLootedFromCreatures`; `MoneyMod` sale siempre `0`.
28. `#LegacyAudit.LOOT.4`: `Player::StoreLootItem` Rust no replica toda la cascada C++ de storage/criteria/`ApplyItemLootedSpell`/guild news/`PROC_FLAG_LOOTED`.
29. `#LegacyAudit.PROFICIENCY.1`: `SetProficiency::default_weapons/default_armor` son tablas fijas no canonicas; C++ aprende proficiencies por `SPELL_EFFECT_PROFICIENCY` y `CanUseItem` no usa una tabla estatica por clase.
30. `#LegacyAudit.MISCWORLD.1`: far teleport/worldport ack no replica la secuencia C++ `SuspendTokenResponse -> NewWorld -> WorldPortResponse -> ResumeToken/AddToMap/initial packets`.
31. `#LegacyAudit.AREATRIGGER.1`: `CMSG_AREA_TRIGGER` Rust omite bits `Entered`/`FromClient` y el handler salta conditions/scripts/quests/tavern/BG/PvP/corpse/transfer-abort C++.
32. `#LegacyAudit.CEMETERY.1`: corregido. `CMSG_REQUEST_CEMETERY_LIST` no consume byte adicional sin equivalente canónico, usa `GraveyardStore` por zona, filtra conditions y solo envia respuesta cuando hay IDs, con maximo 16 como C++.
33. `#LegacyAudit.TAXI.1`: taxi node status Rust solo usa NPC flag; C++ valida creature/hostilidad/nearest node/reaccion/taximask para None/Learned/Unlearned/NotEligible.
34. `#LegacyAudit.QUESTXP.1`: corregido. `QuestXP::calculate_xp` exige fila exacta de `QuestXP` para el nivel y devuelve `0` si falta, como C++.
35. `#LegacyAudit.QUESTXP.2`: corregido. XP minimo escalado aplica `CONFIG_MIN_QUEST_SCALED_XP_RATIO` como C++.
36. `#LegacyAudit.QUESTCMSG.1`: corregido. `CMSG_QUEST_GIVER_QUERY_QUEST` y `CMSG_QUEST_GIVER_ACCEPT_QUEST` leen flags bit (`RespondToGiver`/`StartCheat`) con `read_bit()` como C++ `ReadBit()`.
37. `#LegacyAudit.QUESTPKT.1`: `SMSG_QUERY_QUEST_INFO_RESPONSE` Rust omite el bit `ReadyForTranslation` antes del flush.
38. `#LegacyAudit.QUESTPKT.2`: `QuestRewardsBlock` serializa `QuestChoiceItem` con orden/layout no C++; falta `LootItemType` de 2 bits y `ItemInstance`/`Quantity` estan en orden incorrecto.
39. `#LegacyAudit.QUESTREWARD.1`: reward dialogs alimentan `QuestRewardsBlock` con datos incompletos/no escalados: money crudo, XP/reputacion/currency/skill/faction flags/treasure picker ausentes o cero.
40. `#LegacyAudit.QUESTPKT.3`: `SMSG_QUEST_GIVER_QUEST_COMPLETE` Rust omite `ItemReward` (`ItemInstance`) que C++ serializa al final.
41. `#LegacyAudit.QUEST.1`: `can_take_quest` no implementa `SatisfyQuestTimed` ni `SatisfyQuestBreadcrumbQuest`.
42. `#LegacyAudit.QUEST.2`: `HandleQuestgiverAcceptQuestOpcode` Rust no replica side-effects C++ de item/player sharing, close gossip, `CanAddQuest`, source item/spell, scripts, quest tracker, push-to-party y launch gossip.
43. `#LegacyAudit.QUEST.3`: complete/request/choose reward runtime es parcial: request-items vacio, `CanRewardQuest` incompleto y `RewardQuest` no cubre todas las gates/side-effects C++.
44. `#LegacyAudit.DATASTATS.1`: corregido en issue #60 / PR #118. La ruta live carga `player_racestats` + `player_classlevelstats`, `GtBaseMP`, `CreateHealth=0` y una proyeccion compartida tipo `StatSystem`; la captura C++/Rust coincide en los campos de stats revisados, incluido el 3 % racial via `SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE`.
45. `#LegacyAudit.ITEMSTATS.1`: la aplicacion directa de stats de item en `character.rs` es parcial frente a C++ `_ApplyItemBonuses`: falta AP a ranged desde `ITEM_MOD_ATTACK_POWER`, scaling stat values, ratings separados, regen, spell penetration, extra armor, resistencias por escuela y stats combinados.
46. `#LegacyAudit.SKILL.1`: corregido en issue #62. El indice default exige `Availability == 1` y login aplica `MinLevel`.
47. `#LegacyAudit.SKILL.2`: corregido en issue #62. Default/loaded skills siguen language/level/mono/rank, always-max, DK, tier/step y skills sin abilities.
48. `#LegacyAudit.SKILL.3`: corregido en issue #62. La ruta live usa `LearnSkillRewardedSpells` con niveles/conditions/Riding/masks/skill value reales; captura semantica exacta de 43 spells.
49. `#LegacyAudit.MOVEMENT.1`: corregido. `HandleMovementOpcode` usa el mover actual (`GetUnitBeingMoved()` representado) para validar, actualizar lo representable y emitir `MoveUpdate`; test: `handle_movement_uses_current_mover_guid_like_cpp`.
50. `#LegacyAudit.MOVEMENT.2`: runtime generico de movement omite ramas C++ de teleport-in-progress, `movespline->Finalized()`, alta/baja/reset de transport passenger, vehicle turning con retorno temprano, estado generico completo `Unit::m_movementInfo` para movers no-criatura/no representados y parte del under-map/BG/death flow.
51. `#LegacyAudit.MOVEMENT.3`: `MoveInitActiveMoverComplete` no es 1:1: Rust no ejecuta `UpdateObjectVisibility(false)` como C++ y envia un update parcial inmediato de ActivePlayerData.

## No bugs de comportamiento, pero si deuda de comentario

Referencias de comentario pendientes en el registro histórico; recontrastar antes
de modificar, sin exigir una fase, commit o PR adicional por cada comentario:
...
- `item.rs` `ItemInstance`, purchase/time/enchant serializers, `InvUpdate`, parsers de swap/equip/destroy/cancel/auto-store-bag, e `InventoryChangeFailure` -> `ItemPackets.cpp/h`, `ItemPacketsCommon.cpp/h`, `Player.h`.
...
- `crates/wow-world/src/handlers/character.rs`: quedan pendientes otras rutas fuera de inventory item move/equip/store/destroy/cancel.
...
Orientación de contraste por comportamiento (no una orden de abrir nuevas issues):
...
5. Registrar el resultado en el alcance autorizado; una auditoría no autoriza el fix.
