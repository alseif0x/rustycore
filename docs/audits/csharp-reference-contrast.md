...
| SRP challenge hex casing | `crates/bnet-server/src/rest/handlers.rs:320,512-517,874-879`; test `hex_encode_uses_cpp_uppercase` | `LoginRESTService.cpp:320`; `LoginRESTService.cpp:471-488`; `Util.cpp:849-866` | Bug corregido `#CSharpAudit.BNETREST.4` | C++ usa `AsHexStr()`/`ByteArrayToHexStr()` uppercase para `server_evidence_m2`, `modulus`, `generator`, `salt` y `public_b`. Rust ahora usa uppercase en el helper comun `hex_encode`; el test cubre padding de nibble y letras A-F. |
| Login ticket hex casing | `crates/bnet-server/src/rest/handlers.rs:737-741,874-879`; test `make_login_ticket_uses_cpp_uppercase_hex` | `LoginRESTService.cpp:387-388`; `LoginRESTService.cpp:780`; `Util.cpp:849-866` | Bug corregido `#CSharpAudit.BNETREST.4` | C++ genera `TC-` + 20 bytes uppercase con `ByteArrayToHexStr`. Rust ahora genera el ticket con el mismo helper uppercase y el test valida longitud 40, prefijo `TC-` y ausencia de `a..f`. |
...
| Chat listen ranges | `world-server/src/main.rs`; `wow-network/src/accept.rs`; `wow-world/src/session.rs`; `wow-world/src/handlers/chat.rs`; tests `chat_listen_ranges_use_cpp_world_config_keys`, `say_uses_cpp_configured_listen_range_like_cpp`, `chat_emote_uses_cpp_configured_text_emote_range_like_cpp`, `send_text_emote_uses_cpp_configured_text_emote_range_like_cpp`, `yell_uses_cpp_configured_listen_range_like_cpp` | `World.cpp:1323-1325`; `worldserver.conf.dist:1906-1926`; `ChatHandler.cpp:247-292,713-719`; `Player.cpp:21073-21114` | Bug corregido `#CSharpAudit.CHAT.1` | C++ carga `CONFIG_LISTEN_RANGE_SAY`, `CONFIG_LISTEN_RANGE_TEXTEMOTE` y `CONFIG_LISTEN_RANGE_YELL` desde `ListenRange.*` con fallback de codigo 25/25/300, aunque la config distribuida use Say/TextEmote 40. Rust ahora lee los mismos keys, los propaga a `WorldSession` y los usa para say, yell, `/e` y `CMSG_SEND_TEXT_EMOTE`. |
...
| `PartyInviteServer` valores | `group.rs:840`; `group.rs:947-959`; `party.rs:632-673`; test `party_invite_server_uses_cpp_inviter_values_like_cpp` | `PartyPackets.cpp:62-98`; `PartyPackets.h:60-82`; `GroupHandler.cpp:181` | Bug corregido `#CSharpAudit.PARTY.1` | C++ propaga `packet.ProposedRoles`, `InviterBNetAccountId = session account GUID`, y realm actual/normalizado; `AllowMultipleRoles` queda false por defecto. Rust ahora alimenta esos valores desde `WorldSession` y escribe `ProposedRoles` en vez de cero. |
| `HandlePartyInviteOpcode` validaciones | `group.rs:819-1010` | `GroupHandler.cpp:58-184`; `SocialMgr.cpp:184-191`; `World.cpp:790,913,1163`; `SharedDefines.h:6787-6803` | Bug corregido `#CSharpAudit.PARTY.2` | Rust ya sigue el orden C++ de invite: target/self, `GM.AllowInvite`, `AllowTwoSide.Interaction.Group`, instancia/difficulty, `HasIgnore`, `HasFriend` + `PartyLevelReq`, target grouped/invited, permisos leader/assistant, `Group::IsFull()` 5/40 y OK. `HasIgnore` cubre GUID y cuenta del inviter mediante `character_social` unido a `characters.account`. |
| PartyIndex invite/accept/leave | `group.rs:819-1118`; `group.rs:1293-1430`; tests `party_invite_party_index_instance_does_not_use_full_home_group_like_cpp`, `party_invite_response_party_index_mismatch_keeps_invite_pending_like_cpp`, `leave_group_party_index_instance_does_not_leave_home_group_like_cpp` | `GroupHandler.cpp:114-120`; `GroupHandler.cpp:189-195`; `GroupHandler.cpp:335-357`; `Player.cpp:23447-23460` | Bug corregido `#CSharpAudit.PARTY.3` | Rust ahora conserva `PartyIndex` en invite/accept/leave y lo pasa al lookup representado equivalente a `Player::GetGroup(packet.PartyIndex)`. `PartyIndex=INSTANCE` ya no cae al grupo HOME para full-check, accept ni leave; el invite pendiente se conserva si la categoria aceptada no coincide. |
| `HandlePartyInviteResponseOpcode` lifecycle | `group.rs:1196-1435` | `GroupHandler.cpp:187-238`; `Group.cpp:336-390,406-486`; `Player.cpp:2145-2165` | Bug corregido `#CSharpAudit.PARTY.4` | Rust representa `Player::m_groupInvite` con `PendingInviteLikeCpp`, conserva invite ante `PartyIndex` mismatch, borra invite antes de self/full como C++, revalida `Group::IsFull()`, crea grupo pendiente solo al primer accept con lider online, promueve invitees al grupo creado y aplica `Player::SetGroup` remoto al lider. |
| `HandleLeaveGroupOpcode` lifecycle | `group.rs:1662-1846` | `GroupHandler.cpp:333-357`; `Group.cpp:550-785`; `Player.cpp:2145-2165` | Bug corregido `#CSharpAudit.PARTY.4` | Rust ya resuelve grupo real o `GroupInvite`, rechaza battleground representado con `ERR_INVITE_RESTRICTED`, envia `PARTY_OP_LEAVE` OK antes de remover, y si solo existe pending invite del lider cancela todas las invitees del grupo pendiente como `grpInvite->Disband()`. |
...
| `CMSG_REQUEST_CEMETERY_LIST` | `handlers/misc.rs:2395-2499`; `session.rs`; `world-server/src/main.rs` | `MiscPackets.h:391-397`; `MiscHandler.cpp:367-399` | Bug corregido `#CSharpAudit.CEMETERY.1` | C++ request no tiene campos leidos por el handler, busca graveyards por `zoneId`, filtra conditions y envia hasta 16 IDs; si no hay IDs, no envia respuesta. Rust ya no consume byte C#, recibe `GraveyardStore` cargado con condiciones, responde con IDs de la zona y mantiene `IsGossipTriggered=false`. |
...
8. `#CSharpAudit.BNETSRP.4`: corregido. Login/SRP username y password v1 usan `Utf8ToUpperOnlyLatin` como C++, sin uppercase Unicode completo. Tests: `utf8_to_upper_only_latin_matches_cpp_basic_latin_only`, `srp_username_does_not_apply_unicode_uppercase_like_cpp`.
...
13. `#CSharpAudit.CHAT.1`: corregido. Rangos chat/emote/yell salen de `ListenRange.*` (`CONFIG_LISTEN_RANGE_*`) y se aplican a say, yell, `/e` y `CMSG_SEND_TEXT_EMOTE` como C++.
...
15. `#CSharpAudit.PARTY.1`: corregido. `SMSG_PARTY_INVITE` usa roles propuestos, account GUID, realm actual/normalizado y `AllowMultipleRoles=false` como C++ `PartyInvite::Initialize`.
16. `#CSharpAudit.PARTY.2`: corregido. `HandlePartyInviteOpcode` usa los knobs C++ `GM.AllowInvite`, `AllowTwoSide.Interaction.Group` y `PartyLevelReq`, y aplica `HasIgnore`/`HasFriend` del target en el orden de `GroupHandler.cpp`.
17. `#CSharpAudit.PARTY.3`: corregido. Invite/accept/leave conservan `PartyIndex` y no operan sobre HOME cuando C++ resolveria otra categoria o rechazaria por mismatch.
...
19. `#CSharpAudit.ITEM.1`: corregido. `AutoStoreBagItem` lee `ContainerSlotB` antes de `ContainerSlotA` como C++ y conserva A=source/B=destino.
20. `#CSharpAudit.ITEM.2`: parcial. Handlers inventory move/equip/store ya aplican los gates C++ de `InvUpdate` para counts invalidos, pero siguen direct-inventory-only o simplificados y omiten validaciones/logica C++ de `IsValidPos`, bank access, `CanEquipItem`, `CanStoreItem`, `CanUnequipItem`, `DestroyItem` container-aware y `Player::SwapItem`.
21. `#CSharpAudit.MISC.1`: corregido. `ShowTradeSkill` consume el CMSG como `WorldPackets::Null`/`rfinish()` y no envia `SMSG_SHOW_TRADE_SKILL_RESPONSE`, igual que el handler C++ no-op.
22. `#CSharpAudit.MISC.2`: corregido. `AuctionHelloResponse` omite `AuctionHouseID` y deja delays default 0 como C++ `SendAuctionHello`.
...
25. `#CSharpAudit.LOOT.1`: corregido. `LootResponse` success conserva los defaults C++ `FailureReason=17` y `Threshold=2`; el error setea `FailureReason` especifico pero conserva `Threshold=2`.
...
32. `#CSharpAudit.CEMETERY.1`: corregido. `CMSG_REQUEST_CEMETERY_LIST` no consume byte C#, usa `GraveyardStore` por zona, filtra conditions y solo envia respuesta cuando hay IDs, con maximo 16 como C++.
...
34. `#CSharpAudit.QUESTXP.1`: corregido. `QuestXP::calculate_xp` exige fila exacta de `QuestXP` para el nivel y devuelve `0` si falta, como C++.
35. `#CSharpAudit.QUESTXP.2`: corregido. XP minimo escalado aplica `CONFIG_MIN_QUEST_SCALED_XP_RATIO` como C++.
36. `#CSharpAudit.QUESTCMSG.1`: corregido. `CMSG_QUEST_GIVER_QUERY_QUEST` y `CMSG_QUEST_GIVER_ACCEPT_QUEST` leen flags bit (`RespondToGiver`/`StartCheat`) con `read_bit()` como C++ `ReadBit()`.
...
46. `#CSharpAudit.SKILL.1`: corregido en issue #62. El indice default exige `Availability == 1` y login aplica `MinLevel`.
47. `#CSharpAudit.SKILL.2`: corregido en issue #62. Default/loaded skills siguen language/level/mono/rank, always-max, DK, tier/step y skills sin abilities.
48. `#CSharpAudit.SKILL.3`: corregido en issue #62. La ruta live usa `LearnSkillRewardedSpells` con niveles/conditions/Riding/masks/skill value reales; captura semantica exacta de 43 spells.
