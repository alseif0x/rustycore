...
| `HandlePartyInviteOpcode` validaciones | `group.rs:819-1010` | `GroupHandler.cpp:58-184`; `World.cpp:790,913,1163`; `SharedDefines.h:6787-6803` | Bug runtime / parcial corregido `#CSharpAudit.PARTY.2` | Ya se corrigieron `PartyResult`, target ya agrupado/invite, permisos lider/asistente, `Group::IsFull()` 5/40, GM target con default C++ `GM.AllowInvite=false`, faccion con default `AllowTwoSide.Interaction.Group=false`, instancia y dungeon difficulty. Siguen pendientes social ignore/friend y knobs configurables Rust para `GM.AllowInvite`, `AllowTwoSide.Interaction.Group`, `PartyLevelReq`. |
...
| `HandlePartyInviteResponseOpcode` lifecycle | `group.rs:973-1118` | `GroupHandler.cpp:187-238` | Bug runtime / incompleto | C++ usa `GetGroupInvite()`, quita invite, rechaza self-accept, revisa `IsFull()`, crea grupo con lider online y llama `AddMember()`/`BroadcastGroupUpdate()`. Rust ya respeta el mismatch de `PartyIndex` tras `#CSharpAudit.PARTY.3`, pero sigue modelando un `pending_invites` target->inviter y no revalida full/self ni el lifecycle de `GroupInvite`. |
| `HandleLeaveGroupOpcode` lifecycle | `group.rs:1293-1430` | `GroupHandler.cpp:335-357` | Bug runtime / incompleto | C++ maneja grupo real o invite pendiente, rechaza battleground y envia `SendPartyResult(PARTY_OP_LEAVE, OK)` antes de remover/disband. Rust ya usa `PartyIndex` para no caer a HOME tras `#CSharpAudit.PARTY.3`, pero no cubre cancelacion de pending invite ni envia `PartyCommandResult` OK de leave. |
...
16. `#CSharpAudit.PARTY.2`: parcial corregido. `HandlePartyInviteOpcode` ya cubre valores `PartyResult`, target group/invite, permisos, full-check C++ 5/40, GM default, faccion default, instancia y dungeon difficulty; siguen pendientes social ignore/friend y knobs configurables de `GM.AllowInvite`, `AllowTwoSide.Interaction.Group`, `PartyLevelReq`.
