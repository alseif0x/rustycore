...
| FeatureSystemStatusGlueScreen config | `crates/wow-packet/src/packets/misc.rs:2185-2190`; `crates/wow-world/src/session.rs:30887` | `Handlers/AuthHandler.cpp:104-122`; `World.cpp:951-955`; `World.cpp:1239`; `World.cpp:1597-1599` | Bug de paridad configurable | C++ usa `CharactersPerRealm`, `Expansion`, `CharUndelete`, `BpayStore` y support toggles. Rust fija `max_characters_per_realm=60`, expansion max `2` y support flags false. Correcto solo si config queda en defaults. |
...
| BNet REST form cookie | `crates/bnet-server/src/rest/handlers.rs:254-273` | `src/server/bnetserver/REST/LoginRESTService.cpp:181-188` | Bug confirmado | Rust responde `GET /bnetserver/login/` con `Set-Cookie: JSESSIONID=...`; C++ solo setea `Content-Type` y body JSON. La doc ya lo llamaba carry-over de C#. |
| BNet REST SRP challenge cookie/session | `crates/bnet-server/src/rest/handlers.rs:510-545` | `LoginRESTService.cpp:411-498` | Bug confirmado / diseno divergente | Rust usa cookie `JSESSIONID` para enlazar estado SRP. C++ guarda `Srp` en `session->GetSessionState()` y no emite cookie en el challenge. Puede funcionar, pero no es paridad C++. |
...
| SRP v1 iterations | `bnet_srp6.rs:154-157` | `SRP6.h:165-166`; `LoginRESTService.cpp:468-470` | Bug confirmado | Rust envia `iterations=0` para v1. C++ `BnetSRP6v1Base::GetXIterations()` devuelve `1`. |
| SRP `u = H(A || B)` padding | `bnet_srp6.rs:188-190`; `bnet_srp6.rs:419-429` | `SRP6.h:203-205`; `SRP6.h:226-228` | Bug confirmado | C++ hashea `A.ToByteArray<128/256>(false)` y `B.ToByteArray<128/256>(false)`. Rust hashea bytes big-endian minimos. Si `A` o `B` tienen ceros iniciales, `u`, `S`, `M1` y `M2` divergen. |
| SRP `k` v2+SHA512 | `bnet_srp6.rs:355-360` | `SRP6.h:218-219` | Bug confirmado, rama dormida | C++ tambien paddea `g` a 256 bytes para SHA512. Rust documenta y ejecuta `SHA512(N || g)` sin padding. El REST actual fuerza SHA256, pero la libreria expone SHA512 y C++ lo soporta. |
...
| SRP challenge cuenta inexistente | `handlers.rs:491-492` | `LoginRESTService.cpp:443-449` | Bug confirmado | Rust responde HTTP 400 `Account not found`. C++ responde JSON `authentication_state=DONE` con content-type JSON, sin revelar si la cuenta existe. |
...
5. `#CSharpAudit.BNETSRP.1`: challenge v1 envia `iterations=0`; C++ envia `1`.
6. `#CSharpAudit.BNETSRP.2`: calculo SRP `u` omite padding fijo 128/256 de C++.
7. `#CSharpAudit.BNETSRP.3`: `k` para v2+SHA512 omite padding fijo de C++.
8. `#CSharpAudit.BNETSRP.4`: normalizacion de login/SRP username usa uppercase Unicode en Rust; C++ solo upper Latin basico.
9. `#CSharpAudit.BNETREST.3`: SRP challenge de cuenta inexistente revela error HTTP 400; C++ devuelve `DONE`.
