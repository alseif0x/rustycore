...
4. **`tracing-appender::non_blocking` requires holding a `WorkerGuard`.** If the guard drops (e.g. it was a function-local in `main`), the worker thread shuts down and writes are silently lost. AGENTS.md does not currently document this; `#LOG.2` adds the docs.
