...
| Bank / equipment-sets / void-storage persistence | **WORKS⚠** | personal-bank and equipment/transmog-set persistence are merged; issue #114 locally adds atomic void-storage unlock/deposit/query/swap/withdraw, restores and persists C++ random-affix enchantments, has full fresh-auth relog proof and a byte-clean C++ query capture; GitHub-review hardening now publishes withdrawal item creates plus post-store random/creator/binding VALUES updates and bounds IDs to the 40-bit wire counter, while three false positives were rejected with exact C++ evidence; focused coverage is clean; #114 still needs its repeated full preflight, CI/current-HEAD GitHub review/merge, and aggregate D-C3 remains open until then |
