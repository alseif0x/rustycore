...
| Bank / equipment-sets / void-storage persistence | **WORKS⚠** | personal-bank and equipment/transmog-set persistence are merged; issue #114 locally adds atomic void-storage unlock/deposit/query/swap/withdraw, restores and persists C++ random-affix enchantments, has full fresh-auth relog proof and a byte-clean C++ query capture; #114 still needs final preflight, CI/current-HEAD review/merge, and aggregate D-C3 remains open until then |
