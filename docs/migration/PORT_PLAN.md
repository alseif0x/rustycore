...
- [ ] **L26 C#-reference purge** — re-anchor code + docs to C++. Source: `docs/audits/csharp-reference-audit.md` + `csharp-reference-contrast.md` (51 `#CSharpAudit.*`; ~25 open). Code still cites C# as authority (wow-packet/wow-network/wow-crypto/bnet-server/wow-data); contaminated docs (world-entry-implementation.md, phase4-analysis-and-fixes.md, NPC_VENDOR_FLOW.md, implementation-template.md). Per AGENTS.md "Reference Priority": contrast vs C++, re-anchor comment or file a behavior bug. **Open behavioral findings to turn into issues / fold into Part 1:** SKILL.1/2/3 (starting skills/spells vs LearnDefaultSkills), CHARACTER.1 (enum flags), CHARACTER.2 (logout always instant → M5.5), AREATRIGGER.1 (skips conditions/scripts → M4.7), guid.rs HasEntry always-true (core), QUESTPKT.1/2/3 + QUESTREWARD.1 (quest packet layout → M4.7). **Tracked as issues #50–#64; index issue #65**. ITEM.2 / issue #52 is merged with C++-anchored move/equip/store validation and strict action captures. DATASTATS.1 / issue #60 has a local C++-anchored implementation pending PR: `player_racestats` + `player_classlevelstats`, `GtBaseMP`, shared create/login/equipment/level-up StatSystem projection, login-order parity for passive parry/block, persisted stat auras and the saved-health clamp, plus exact field-level C++/Rust capture evidence including the total-stat-percentage racial passive. Neither slice closes L26 or their broader item/aura parity ledgers.
