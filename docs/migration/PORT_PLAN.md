...
- [ ] **L26 C#-reference purge** — re-anchor code + docs to C++. Source: `docs/audits/csharp-reference-audit.md` + `csharp-reference-contrast.md` (51 `#CSharpAudit.*`; ~25 open). Code still cites C# as authority (wow-packet/wow-network/wow-crypto/bnet-server/wow-data); contaminated docs (world-entry-implementation.md, phase4-analysis-and-fixes.md, NPC_VENDOR_FLOW.md, implementation-template.md). Per AGENTS.md "Reference Priority": contrast vs C++, re-anchor comment or file a behavior bug. **Open behavioral findings to turn into issues / fold into Part 1:** DATASTATS.1 (player_stats uses C# tables vs player_classlevelstats+GtBaseMP+StatSystem → M0.1/M3), SKILL.1/2/3 (starting skills/spells vs LearnDefaultSkills), CHARACTER.1 (enum flags), CHARACTER.2 (logout always instant → M5.5), AREATRIGGER.1 (skips conditions/scripts → M4.7), guid.rs HasEntry always-true (core), QUESTPKT.1/2/3 + QUESTREWARD.1 (quest packet layout → M4.7). **Tracked as issues #50–#64; index issue #65**. ITEM.2 / issue #52 now has a locally capture-clean C++-anchored implementation for the scoped inventory move/equip/store gates, pending its PR CI, current-HEAD reviewer verdict and merge; this does not close L26 or broader item parity. The audit docs themselves remain local/uncommitted.
