...
canonical/legacy reciprocal references. Implementation: `3cba7f7f`. The
behavior-preserving replay extraction `027067eb` retains the owner, lock order
and public paths while reducing `creature_melee_tick.rs` from 2,135 to 1,863
...
branch:** `Unit::IsImmunedToDamage` checks both school and damage-immunity masks;
the melee owner now applies the same full-normal-school check to player and
creature victims from their represented aura effects. This slice keeps the
existing school mask path and adds the missing `SPELL_AURA_DAMAGE_IMMUNITY`
consumer. Split damage, richer spell-immunity registry data, crushing policy and
live DB/relogin acceptance remain open.
Validation on this candidate: the focused player-victim damage-immunity
regression, the existing melee suites, production `world-server` check,
formatting, diff check, physical ratchet and syntax-only ownership check pass.
The general architecture check remains red only on the known hotspot ratchet;
the added test is isolated in a bounded scenario file and introduces no new
...
for ordinary eligible levels; the Rust path preserves that expression and does
not invent a crushing blow or silently repair the target source. The outcome
switch and wire presenter retain the exact 150%/`HITINFO_CRUSHING` branch for
fidelity if a positive raw band is ever supplied. Split damage remains the next
behavioral boundary because it needs a second canonical damage target plus
`DealDamage` ordering, immunity, combat-log and proc ownership; live DB/relogin
acceptance is also still open. This slice does not close #29.
...
planner into production (`Player.cpp:22093-22094`). `HandleShapeshiftBoosts`
remains open. wow-data --lib 753/0, wow-entities --lib 940/0, wow-world --lib
3953/0/1; no live DB/restart/relogin QA.
...
victims keep it unapplied. wow-data --lib 752/0, wow-world --lib 3933/0/1; no
live DB/restart/relogin QA.
...
`CheckAttackFitToAuraRequirement`. The weapon-enchantment damage term stays open.
wow-data --lib 752/0, wow-entities --lib 940/0, wow-world --lib 3928/0/1; no live
DB/restart/relogin QA.
...
`ActivePlayerData::VersatilityBonus`, now published through the canonical
snapshot, the create block and the narrow values update (bit 56). The handler's
commented-out healing/versatility terms stay absent, matching C++.
...
`SPELL_AURA_MOD_HEALING_DONE_PERCENT` (136) effects and published through the
canonical snapshot, the create block and the narrow values update (bit 61).
`ModTargetResistance`/`ModTargetPhysicalResistance` remain the next gate.
wow-data --lib 751/0, wow-entities --lib 940/0, wow-world --lib 3926/0/1,
wow-packet --lib 742/0; no live DB/restart/relogin QA.
