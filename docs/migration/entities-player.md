...
The Player module is **not split per-feature** in this branch. It is one monolith with neighbour managers.
...
- `crates/wow-world/src/session.rs` — `WorldSession` remains a large transitional session/application owner with many represented player-state mirrors. Gossip menu contents stay in `gossip_options`, while their source/trainer provenance has one private C++-shaped owner, `PlayerInteractionDataLikeCpp { source_guid, trainer_id }`. Canonical playable state also exists in `wow_entities::Player` through `SessionPlayerController`; this file is not the sole player model.
