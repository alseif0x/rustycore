...
- **Status — mostly done via staged refactor (A-E); one bounded follow-up left.** The login init was factored into reusable `send_initial_packets_before_add_to_map` / `send_initial_packets_after_add_to_map(&mut self)` helpers (character.rs), a single `Unit::m_movementCounter` (`movement_counter_like_cpp`) replaced the two split sequence counters, and the login spell-history snapshot is persisted on the session. `handle_world_port_response` (misc.rs:2094) now: sends `ResumeToken` with the **real movement counter** (read before the before-add reset) and Reason 1; replays the before-add control packets (reset counter + `SMSG_MOVE_SET_ACTIVE_MOVER` + fresh time sync); refreshes nearby objects (AddToMap-equivalent); then calls the **full `send_initial_packets_after_add_to_map` helper** — post-add phase shift, **`InitWorldStates` resolved for the DESTINATION map** (the high-value cross-map fix, also carrying the 1232 PvP-season values), the PhasingHandler::OnMapChange phase shift, CUF profiles and auras. Regression: world_port_response_clears_far_teleport_semaphore_like_cpp asserts the full NewWorld/ResumeToken/MoveSetActiveMover/TimeSyncRequest/PhaseShiftChange/InitWorldStates/PhaseShiftChange/LoadCufProfiles order. **Remaining (bounded follow-up):** the full **before-add** packet set (spells/factions/action-bars via the before-add helper) is intentionally NOT re-sent on teleport — the client retains it from login unchanged, and calling the DB-backed before-add helper from the teleport unit test deadlocks/hangs in the no-DB test env (needs dedicated diagnosis before reuse). Also still open: destination-transport re-seat (depends on 1230) and the seamless (Reason 2) path. **Needs live far-teleport validation.**
- **Update — far teleport LIVE-VALIDATED working (2026-06-27).** Live testing found the worldport flow broken at three more PRE-EXISTING points (all now fixed): (1) `CMSG_WORLD_PORT_RESPONSE` had an inventory entry but no dispatcher match arm in session.rs — the handler never ran live; added the arm. (2) `CMSG_SUSPEND_TOKEN_RESPONSE` was unhandled, so `SMSG_NEW_WORLD` was never sent and the client hung at 0%; added `handle_suspend_token_response` that sends NewWorld (C++ HandleSuspendTokenResponse, MovementHandler.cpp:253), and moved the NewWorld send OUT of handle_world_port_response (it belongs before the ack). (3) `SuspendToken` used a hardcoded sequence_index=1 while `ResumeToken` used the counter; C++ pairs them on `m_movementCounter` (Player.cpp:1466), so the mismatch left the client at 80% — both now read the counter. Finally, SendInitSelf (the player's own ActivePlayer create for the destination map; C++ Map::AddPlayerToMap initPlayer=true, Map.cpp:470) + skill_info/languages were added to the worldport ack (client hung at 100% / "can't speak" until added). Player now teleports cross-continent, enters the world, moves and chats. Remaining for full fidelity: re-send the full inventory item objects on teleport (session holds only a minimal InventoryItem, so a DB re-load is needed) — currently moot behind a SEPARATE pre-existing inventory-display bug (the backpack itself won't open even at login; suspected ActivePlayerData serialization, e.g. the transmog=5528 dynamic field).
...
- **C++ confirmed:** These are realm-wide WorldStateMgr values, not constants: WS_CURRENT_PVP_SEASON_ID (3191) = `in_progress ? CONFIG_ARENA_SEASON_ID : 0`, WS_PREVIOUS_PVP_SEASON_ID (3901) = `CONFIG_ARENA_SEASON_ID - in_progress` (World.cpp:1363-1364, 2300-2301; SharedDefines.h:8081-8082). `Arena.ArenaSeason.ID` defaults to 32, `Arena.ArenaSeason.InProgress` to false. The captured run had season 32 in progress -> 3191=32, 3901=31.
- **Status — done.** Added `apply_pvp_season_world_states_like_cpp` (character.rs) which overrides 3191/3901 in the built InitWorldStates list (in place, preserving order; appends if absent), called from `load_initial_world_states_for_login_like_cpp` with the values read from config (`Arena.ArenaSeason.ID` default 32, `Arena.ArenaSeason.InProgress` default 0/false). Added `wow-config` to wow-world deps. Regression: `pvp_season_world_states_match_cpp_world_state_mgr` (in-progress 32 -> 32/31; not-in-progress -> 0/32; absent -> appended). Checks: cargo fmt --all --check (0); cargo test -p wow-world --lib (2548/0); cargo check -p world-server.
