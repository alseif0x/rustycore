...
| T3 complete | Character | apply returned `Added`/`Replayed` (pet durable, fence revalidated by the owner), or a receipt re-check proves the pet durable for a `CompensationPending` row | guarded `UPDATE status=Completed WHERE key AND status IN (PendingApplication, CompensationPending)` | `request_key` + status guard | 0 rows ⇒ read status: `Completed` ⇒ idempotent ok | command still pending; recovery re-completes | status read is decisive | recovery completes, publication stays silent (at-most-once) |
...
**Saga invariants.** (1) Money is deducted at most once per `request_key` (guarded update + PK in one tx). (2) At most one pet per `request_key` (Login DB receipt PK + owner replay). (3) Refund at most once (refund + status flip in one tx). (4) No publication before the pet is durable, no publication on compensation/failure, exactly one `SMSG_BATTLE_PET_UPDATES` petAdded per successful command (`Added` publishes once after T3; `Replayed` and post-T3 recovery stay silent). (5) The dependent runtime spell learn is runtime-only, mirroring C++ `dependent=true` (never a `character_spell` row). (6) All recovery is durable-state driven — no in-memory flags decide whether a charge, pet or refund already happened. (7) No lock guard crosses `.await` except the pre-existing async per-character money mutex (#159 pattern); the saga spawns no tasks of its own, and the only worker is the #160 owner worker drained by the registry's bounded shutdown.
