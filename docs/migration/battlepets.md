...
**Saga invariants.** (1) Money is deducted at most once per `request_key` (guarded update + PK in one tx). (2) At most one pet per `request_key` (Login DB receipt PK + owner replay). (3) Refund at most once (refund + status flip in one tx). (4) No publication before the pet is durable, no publication on compensation/failure, and exactly one `SMSG_BATTLE_PET_UPDATES` petAdded plus one dependent learned spell per successful command: publication runs after the durable pet exists and before the completion record, guarded by the durable `published` marker that the completion flip also sets, so a crash between the durable pet and the completion still yields the one publication at recovery instead of losing it. The only residual duplicate window is a crash between the packet send and the marker commit, which replays one idempotent petAdded upsert on the next login. (5) The dependent runtime spell learn is runtime-only, mirroring C++ `dependent=true` (never a `character_spell` row). (6) All recovery is durable-state driven — no in-memory flags decide whether a charge, pet or refund already happened. (7) No lock guard crosses `.await` except the pre-existing async per-character money mutex (#159 pattern); the saga spawns no tasks of its own, and the only worker is the #160 owner worker drained by the registry's bounded shutdown.
