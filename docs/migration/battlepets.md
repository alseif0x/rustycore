...
Deliberate deviation recorded for review (remote PR #172 P1, intentionally NOT implemented): the reviewer asked login to restore the purchased dependent trainer spell from journal ownership. C++ makes that spell intentionally ephemeral — `LearnSpell(spellId, dependent=true)` exists precisely so the trainer spell is never saved (`Trainer.cpp:140-145`: "marking as dependent prevents saving the spell to database (intended)"; `_SaveSpells` skips dependent rows at `Player.cpp:20437-20448`). After every relog C++ shows the spell as available again and the player can buy another pet of the same species up to the cap (`DEFAULT_MAX_BATTLE_PETS_PER_SPECIES = 3` proves repeat purchases are intended; `GetSpellState` only blocks on `HasSpell`, `Trainer.cpp:185-188`). Reconstructing the spell at login would block that C++ re-buy behavior and invent a durable spell record C++ deliberately never writes. The pet — the only durable proof of purchase — is already delivered by the journal load, exactly like C++.
