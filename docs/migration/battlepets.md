...
**Deliberate bounded correction.** Rust keeps every C++-observable rule above but closes the two-commit window with a durable saga keyed by a 128-bit `request_key` shared with the #160 Login DB receipt (`battle_pet_add_requests`): (T1) one Character DB transaction deducts the money with a guarded `UPDATE ... WHERE guid=? AND money=?` and inserts the pending command — the unique key makes a replayed attempt impossible to charge twice; (T2) the command is applied through `BattlePetAccountOwnerLikeCpp::try_add_pet_like_cpp`, whose Login DB transaction revalidates the fence, the journal lease and the per-species capacity and writes pet + receipt together, so a replay returns the original pet instead of creating another; (T3) completion is recorded in Character DB after the publication claim, and the durable `published` claim commits before the packets emit, so a success publishes exactly once; a `Completed` row with a clear marker is the recovery-publication signal and is deliberately included in the recovery scan; (T4) a terminal apply failure is recorded as `CompensationPending`; (T5) the refund and the `Compensated` flip commit in one Character DB transaction, so the refund executes exactly once; (6) login recovery converges any interrupted command. Sanctioned behavior deltas vs legacy: admission-time capacity/journal-lock failures return a typed structured result (the wire stays silent exactly like the C++ cap case; no C++ case exists for the journal lock) instead of being indistinguishable from a successful purchase internally, and the trainer list now renders a purchasable battle-pet spell as available because C++ `GetSpellState` has no cap gate. A castable trainer spell with a confirmed species keeps the normal wrapper acquisition but retains the C++-shared silent cap gate and visual-kit suppression (`Trainer.cpp:99-109,121-125`), because C++ resolves the species before `IsCastable()`.
...
Deliberate deviation recorded for review (local Codex publication-order findings, intentionally deferred with C++ evidence): the success publication uses outbox order — the durable `published` claim commits after the durable pet exists and BEFORE the packets emit. A crash/cancellation strictly between the claim and the emit loses the immediate `SMSG_BATTLE_PET_UPDATES`/`SMSG_LEARNED_SPELLS` notification and the session-scoped dependent runtime learn. That loss is C++-identical, not a regression: the dependent spell is intentionally never persisted (`Player.cpp:20437-20448`), so C++ itself loses the learn on ANY disconnect and even lets the player re-buy the same species after relog until the per-species cap — restoring the learn at login would instead block that C++ re-buy behavior. The durable pet still reaches the client through the journal load/`SendJournal` at the next login, exactly like C++ after a crash. The alternative order (emit before claim) was rejected: it can replay the packets and criteria side effects on the next login, and its recovery re-emit would re-learn the dependent spell after relog, diverging from C++ re-buy semantics. Both orders have one irreducible crash window because a packet send and a DB commit cannot be atomic; claim-before-emit is the window whose content matches legacy crash semantics.
...
|---|---|---|---|---|---|---|---|---|
| T1 charge+command | Character | admission revalidation passed (membership, gates, price, money, species metadata, selection inputs, capacity, journal lease) under the exclusive money guard | one tx: guarded money `UPDATE` (expects 1 row) + `INSERT` command `PendingApplication` (expects 1 row) | `request_key` PK | same-token retry self-reconciles (guarded UPDATE fails, INSERT hits PK, row presence proves commit) | no charge, no command; client retry gets a fresh key | reconcile by `SELECT` on `request_key`: row ⇒ committed, none ⇒ rolled back, else quarantine | recovery treats row as `PendingApplication` |
...
| T3 complete | Character | apply returned `Added`/`Replayed` (pet durable, fence revalidated by the owner) and the success publication ran, or a receipt re-check proves the pet durable for a `CompensationPending` row | guarded `UPDATE status=Completed WHERE key AND status IN (PendingApplication, CompensationPending)`, preceded by the guarded `published` marker write | `request_key` + status/marker guards | 0 rows ⇒ read status: `Completed` ⇒ idempotent ok | command still pending; recovery re-completes | status read is decisive | recovery completes and republishes only when `published=0` — a replayed receipt whose live execution died before completing still gets its one success publication |
...
| T5 compensate | Character | `CompensationPending` | one tx: refund `UPDATE characters SET money=LEAST(money+price, MAX)` (expects 1 row) + `UPDATE status=Compensated` (expects 1 row) | status flip in the same tx as the refund | replay finds `Compensated` ⇒ no-op, refund cannot run twice | refund not applied; recovery retries | reconcile by status: `Compensated` ⇒ committed, `CompensationPending` ⇒ rolled back | recovery retries until terminal |
...
**Saga invariants.** (1) Money is deducted at most once per `request_key` (guarded update + PK in one tx). (2) At most one pet per `request_key` (Login DB receipt PK + owner replay). (3) Refund at most once (refund + status flip in one tx). (4) No publication before the pet is durable, no publication on compensation/failure, and exactly one `SMSG_BATTLE_PET_UPDATES` petAdded plus one dependent learned spell per successful command: outbox order — the durable `published` claim commits after the durable pet exists and BEFORE the packets emit, so recovery can never repeat an already-emitted publication; a completion that outlived an unclaimed publication is republished by recovery (the recovery scan includes `Completed` rows whose marker is clear). The residual window is a crash between the claim commit and the packet send, which loses the immediate notification exactly like a C++ crash loses an unflushed packet — the durable pet still reaches the client through the journal at the next login. (5) The dependent runtime spell learn is runtime-only, mirroring C++ `dependent=true` (never a `character_spell` row). (6) All recovery is durable-state driven — no in-memory flags decide whether a charge, pet or refund already happened. (7) No lock guard crosses `.await` except the pre-existing async per-character money mutex (#159 pattern); the saga spawns no tasks of its own, and the only worker is the #160 owner worker drained by the registry's bounded shutdown.
