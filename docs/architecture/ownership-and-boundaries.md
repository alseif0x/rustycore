...
The 2026-09-05 plan review at `93e4002a` retains #578 as one coherent ownership
macro-deliverable under #133. Internal commits and regression checkpoints are implementation
boundaries, not field-by-field issues or automatic PR closeouts. #578 owns the remaining
Player/Map lifetime, transition, handler and resource-boundary decisions described below;
#153 only verifies that the completed decisions and their evidence satisfy #133. Known
implementation or design work must not be assigned to the terminal audit.
...
inside the existing macro, not new issues or a field-by-field execution queue.
...
exceptions keep `tracking_issue` 133 only while the parent genuinely remains open and a concrete
decision remains in its authorized implementation scope. The remaining Session/handler/transport
boundary decisions name #578. The ledger's `reaudit_issue` (#153) verifies final classifications
...
contracts above, the #578 checkpoint and the source/module-design ratchets.
...
Under the 2026-09-05 plan, #578 owns inherited unresolved boundary decisions and #153 verifies
their completed classification. The preserved source annotations and persistence artifacts still
carry that historical target: #578 C4 must review each semantic boundary and reconcile source
...
are parents rather than PRs; external QA prerequisite #177 is separate. The four lanes retain
historical completed steps as well as the current macros:
...
- runtime/persistence authority: #188–#200 and completed follow-up #204, with remaining convergence in #578;
