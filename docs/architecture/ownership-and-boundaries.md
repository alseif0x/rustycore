...
| Represented player gameplay state | `wow_entities::Player` is the generation-checked owner of the migrated gameplay families, including C++ `Player::_CUFProfiles`, `_equipmentSets`, `_voidStorageItems`, `_specializationInfo`, `mSkillStatus`, guild membership/`m_GuildIdInvited`, `m_trade`, `m_atLoginFlags`, weapon/armor proficiencies, `m_ArenaTeamIdInvited` and represented `m_bgData` context/queue slots, plus their coherent-load markers; remaining transitional families are still explicit `wow_world::WorldSession` fields | migrated handlers query/mutate the exact `PlayerHandle`; remaining handlers and session update code still own only their uncut families | packet builders, persistence helpers and `PlayerRegistry` summaries consume typed snapshots/outcomes | selected-character lifetime under `wow_map::MapManager`, including explicit detached far-teleport residence; the former whole-`Player` Session→Map snapshot/write-back bridge is gone and unresolved/stale handles remain unknown rather than fabricating empty profile, saved-set, vault, specialization, skill, guild, trade, capability or battleground context | #578 completes the remaining family cuts, Map authority and resource decomposition as one macro-deliverable; #153 performs the terminal audit rather than deferring a known ownership bridge. |
