...
12 `cfg(test)` fixtures. It also records all 46 logical impl owners and 3,285 exact
associated-item signatures rather than freezing the number of physical `impl` blocks. Issue #408
retires three statement helpers plus the statement-shaped plan and replaces them with one semantic
request plan, a net reduction of three signatures. Private composition-side `SessionResources` has 248 fields,
...
589 exact AST rows. After #408 moved the talent-reset transaction behind the lifecycle port,
the workspace-wide persistence inventory contains 21,679 exact rows—11,543 production and 10,136
test-fixture—with multiplicity 23,967 (13,104 production and 10,863 test). The reviewed delta
removes 45 production and 68 test concrete-access rows from the five `wow-world` talent-reset
workflow groups; the existing stable `wow-database` adapter now owns all 52 production rows for
statement construction, transaction execution and exact-money COMMIT reconciliation, while seven
adapter test rows freeze that boundary. Its workflow annotations fall from 868 to 865, while
workflows targeting #169 fall from 23 to 19. Six generated-source
...
53. #189 — durable loot persistence coordination;
54. #192 — runtime/fanout directory consumers;
55. #193 — combat/loot directory consumers;
56. #194 — quest/spell/movement directory consumers;
57. #197 — atomic group invite/create transitions;
58. #198 — atomic group membership/leadership transitions;
59. #199 — Group persistence/publication closure;
60. #195 — social/group session addressing;
61. #196 — PlayerRegistry storage closure;
62. #138 — opaque session-directory relocation;
63. #191 — mailbox protocol relocation;
64. #137 — encapsulated Group owner move;
65. #190 — durable creature-runtime rail relocation;
66. #140 — Session mailbox pump;
67. #252 — retire the temporary PlayerBroadcastInfo gameplay mirror;
68. #182 — logical realm/instance routing;
69. #183 — Session-only phase driver;
70. #184 — login/logout lifecycle modules;
71. #224 — character/loot/quest physical modules;
72. #225 — Map/MapManager physical modules;
73. #226 — Player/Unit physical modules;
74. #227 — packet/spell-data physical modules;
75. #228 — trusted linked external module API;
76. #229 — deterministic external Cargo composition;
77. #230 — agent-neutral module CLI and skeleton;
78. #231 — typed module configuration/fixtures;
79. #270 — retire the four PlayerBroadcastInfo transport endpoints;
80. #359 — single dispatch mechanism for every opcode;
81. #297 — promote the Session kernel to `wow-session`;
82. #378 — move the remaining five session modules into `wow-session`;
83. #153 — terminal architecture audit.
