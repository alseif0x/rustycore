...
| Immutable spell-acquisition projection | `wow_world::spell_acquisition` owns the pure fixed-point algorithm and its value-only snapshot/plan types; the live player remains the state owner | no runtime writer: planning mutates only a private ordered copy | #157 consumes ordered primary-profession outcomes, #158 consumes the same durable transition/action plan, and #159 rechecks the inputs under the canonical owner | one trainer operation; a `WorldSession` adapter succeeds only for complete spell/skill rows and exact slot occupancy. Static cast safety is separate from the per-player `reached_immediate_phase` / HIT_TARGET effect mask, and future-player-condition results are an occurrence-sensitive causal tape rather than one value per ID; see [the detailed contract](spell-acquisition-plan.md) | Retire the specialized planner only when canonical `Player` methods expose the same atomic dry-run/apply contract. Never reconstruct capacity or criteria from a flat trigger list, sort profession outcomes, or infer “no immunity” from missing runtime state. |
