...
| Session mailbox and durable rails | private `wow_world::session::mailbox::{protocol, durable, pump}`: `SessionCommand` plus every ordinary payload, `SharedClientVisibleGuidsLikeCpp`, the durable creature-runtime and loot-money rails, and the single pump | session/global runtime and gameplay producers enqueue commands; one session task consumes them | the owning session consumes FIFO commands and publishes acknowledgements | connection/session lifetime; queue identity, capacity, FIFO, incarnation fences, acknowledgements, and shutdown drain are observable | #191 and #190 completed their bounded protocol/rail relocations, #138 closed broad directory access, and #140 moved the complete vertical without changing queue identity, FIFO, durability, capacity, incarnation fences, acknowledgements or shutdown drain. `wow-network` now owns only transport primitives. #189 still removes durable loot persistence coordination. |
