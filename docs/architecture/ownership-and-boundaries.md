...
requires a new future closure.

**Allocation history:** the 2026-09-06 revision moved unfinished C0–C4 ownership,
dependency, runtime and physical exits from the bounded #578 foundation to #584.
The dated inventories below retain their original evidence identity;
remaining implementation belongs to analyzed coherent macros under #584, while
#153 audits the completed decisions. Closed checkpoints are evidence, not current
instructions to repeat a predecessor.
...
The 2026-09-05 review at `93e4002a` established the coherent-operation acceptance
retained by the current #584 core program. Internal commits and regression checkpoints
are implementation boundaries, not field-by-field issues or automatic PR closeouts.
#584 owns remaining Player/Map lifetime, transition, handler and resource decisions;
#153 verifies the completed decisions and evidence against the approved architecture
contract. Known implementation or design work must not be assigned to that audit.

Judge progress by complete transitions and retired authority: a single canonical mutable owner,
coherent inputs, explicit lifetime/phase admission, classified persistence outcomes and ordered
...
selective `hecs`, retaining cohesive domain aggregates. Production storage migration requires
the finite independent-module/native-Wasm conformance proof in that plan and the affected
real-owner integration under #584. The selection is made; a demonstrated backend-specific
...
#583, under #99 and after the delivered #231/#578 foundations plus required #584
core, delivers shared semantic hooks for native first-party/custom modules and a bounded
Wasm executor with Rust/C bindings, full stateful composition, durability and operator
lifecycle proof. Wasm is optional for the operator to enable, but its #583/#153 acceptance
is required. #153 audits the completed core and selected module product, not the entire
future #99 ecosystem. This bounded Wasm delivery does not wait for M6; independent
gameplay need not wait for every SDK step. Complete architecture/module acceptance
is required for the Part-1 program exit at #47. Core does not depend on the production
SDK/Wasm executor. No new publication, deployment or destructive authority follows.

The C0–C4 acceptance inherited by #584 comprises **C0 phases contract**, **C1 lifetime/save**,
**C2 complete operations**, **C3 runtime/delivery**, and **C4 inherited boundary
decisions + acceptance**. C0 records the actual before/after contract before an ownership or
timing change; it does not require implementing a new scheduler first. C1 closes complete
lifetime and persistence transitions; C2 migrates complete use cases and their readers;
C3 converges the remaining runtime/publication responsibilities; C4 resolves inherited
adapter decisions and gathers macro acceptance evidence. These are regression checkpoints
inside coherent implementation macros, not new issues or a field-by-field execution queue.

Use the [reanalysis checkpoints](modularity-and-ecs-plan.md#reanalysis-checkpoints--evidence-before-replication):
conformance before production storage migration; first real C1/C2 operation with C0 evidence
before replicating its pattern; complete C4 balance before #583 production integration; #153
after both macros merge. The later #47/M6.2 checkpoint reviews the whole port before Part 2.
These are evidence reviews inside authorized work, not additional routine approval gates.
...
|---|---|---|
| `dispatch` + `registry` | The registered thunk takes `&mut WorldSession`, borrowed `SessionHandlerCatalogsLikeCpp` and `WorldPacket`; the sole registry lives with its concrete consumer in `wow-world`, while `wow-handler` supplies the shared vocabulary. | #584 owns thin handlers over typed Player/Map transitions and the final registry/adapter classification. Keep the single registration/call mechanism; a `PacketRouter` trait or kernel relocation is not required merely to erase a dependency exception. #153 verifies the completed boundary. |
| `admission` | Packet-rate accounting is session work; spoof-ban completion now uses the SQLx-free `PacketSpoofBanPersistencePortLikeCpp`, not `LoginDatabase`. Its persistence outcome and ordered Session application remain outside the transport kernel. | The persistence port is already present. #584 owns any remaining pure-decision/application separation with complete success, failure and missing-authority outcomes; do not reopen the completed SQL extraction or default a missing authority. |
| `lifecycle` | Character claims, canonical map/accessor teardown, loot fences and durable Player persistence are application/gameplay lifecycle, matching C++ `WorldSession::LogoutPlayer`, not transport state. | #584 owns the complete active/detached Player lifetime and canonical transition cuts while preserving the persistence contracts established by #169. Keep the ordered application adapter here; a state-machine extraction must name every owed transition, persistence and publication step. |
| `driver` | The pass still interleaves packet ingestion with Player timers, aura/spell/gameobject work, creature compatibility ticks, saves, logout and dispatch. That current mix is not evidence that map-owned gameplay executes in its final C++ phase. | #584 owns remaining phase/admission and gameplay-scheduling convergence, preserving #28/#371's existing owner cuts and distinguishing structural moves from separately evidenced timing changes. The transport kernel may return connection outcomes, but must not schedule gameplay. #153 verifies the final shell and phase evidence. |
| `mailbox` | `SessionCommand` is a typed gameplay protocol and the durable rail carries creature/combat results; moving it would import domain payloads into P4. | #584 owns complete command-consumer migrations and the resulting mailbox classification. Preserve durable obligations, ordering and backpressure; introduce a payload-agnostic queue port only if the resulting real boundary earns it. #371 changed the legacy clock, not these payloads; #153 verifies rather than designs the final seam. |
| `directory` | Presence/addressability, generation fences, delivery handles and the bounded loot-roll identity list are session concerns; gameplay reads now resolve from canonical Player/Map owners. | #252 retired the gameplay mirror; #584 owns the remaining narrow presence/address classification. A detached Player remains a valid lifetime owner, while an active-map query may be unavailable; stale/missing ownership never becomes a fabricated default. #153 verifies the completed distinction. |
...
A migration PASS does not complete #584's remaining splits. The **logical view** counts each curated
...
The policy's workspace and external `exceptions` are a **ratchet, not an endorsement of
unfinished authority**. A coarse package category can also require an exception for a legitimate
application adapter: `wow-world` consumes handler vocabulary, packet DTOs, transport contracts and
its extracted `wow-session` kernel. Classify those uses against the real consumer, separately from
gameplay reaching infrastructure or retaining mutable ownership in Session. A reviewed final
adapter edge may remain; creating a crate or trait solely to delete it is not required. Each
exception:
...
exceptions may name an epic only while that owner remains open and a concrete
decision remains in its authorized implementation scope. The remaining Session/handler/transport
boundary decisions belong to #584. The ledger's `reaudit_issue` (#153) verifies final classifications
and retirement evidence; it must not design an inversion, implement a missing port or create
follow-up work as a substitute for completing the known cut.
...
| Reserved-but-empty crates | `wow-achievement`, `wow-combat`, `wow-pvp` and `wow-spell` (`domain-runtime`) contain no code | nothing writes them yet | nothing reads them, and nothing they read either: #298 removed the phantom Cargo edges the four still declared after #288 removed `wow-ecs`'s | none - they hold no state and run on no clock | The reservation is now checked, not described: `dependency-policy.json#reserved_packages` names the owning issue for each (`wow-combat` #29, `wow-spell` #30, `wow-pvp` and `wow-achievement` #48), `check_architecture.py` fails when one acquires a dependent or a dependency and when its owning issue closes with the crate still empty, and `refresh-issue-state` keeps that owner state derived rather than hand-maintained. `wow-ecs` was removed. The later selective-hecs decision belongs privately inside `wow-map` under #584; it preserves cohesive Player/domain aggregates and does not require recreating that crate. |
| Database migration authority | `rustycore-db` is the sole runtime-distributed schema writer; `wow_database::migration` owns MariaDB history, advisory locks and statement execution, while the source-controlled manifest owns immutable `(component, database, version)` identities | operators invoke `status`, `migrate --dry-run`, `migrate`, or `validate`; servers consume only the embedded manifest and bounded read-only compatibility check | `world-server` validates all four databases and `bnet-server` validates auth before any runtime write or listener | migration is an explicit administrative lifecycle; server startup never creates a database, scans SQL paths, invokes a client, downloads content, or executes migration DDL/DML | #256 replaces the permissive startup `DbUpdater`, preserves an exact legacy-history transition, and isolates future module versions by component namespace. Baseline acquisition remains #255; #153 retains the terminal persistence-boundary audit. |
...
| Player mail and next-mail query | canonical `wow_entities::PlayerGameplayState::mails`, hydrated by the existing player-lifecycle capability during login | the lifecycle adapter alone owns `CHAR_SEL_MAIL`, its player-guid bind and row decoding; the generation-checked `PlayerHandle` is the only runtime access path | the player handler retains clock comparison, read/delivery filters, sender dedupe, three-entry cap, packet construction and Realm routing | selected-character lifetime, including explicit detached far-teleport residence; a missing/stale owner remains unknown and cannot be replaced by an empty mail list | #578 retires #460's on-demand query and follows C++ `PlayerLoginQueryHolder` plus `_player->GetMails()` ownership. |
| ObjectMgr world-query catalogs | immutable `wow_data::{CreatureQueryCatalogLikeCpp, GameObjectQueryCatalogLikeCpp, PageTextCatalogLikeCpp}` composed once before listeners and grouped with the existing gameobject-lifecycle store in one required `ObjectMgrCatalogsLikeCpp` capability | `wow_database::world_query_catalog_adapter` alone owns the complete World reads and row decoding; `world-server` validates construction and publishes the typed capability | creature/gameobject/page-text query, gameobject-use/money and lifecycle consumers read the immutable projections; quest-item rows reuse the existing startup store | process lifetime; base rows, sparse locale overlays, display order, page-chain bounds and gameobject addon money are frozen before sessions start | #578 retires the four #462/#480/#482/#484 on-demand ports and their five concrete adapters, reduces `SessionResources` 273→268 and replaces rather than enlarges the existing Session catalog slot. |
| Canonical map-corpse persistence capability | `wow_persistence::MapCorpsePersistencePortLikeCpp` owns the SQLx-free `(map, instance)` request, raw persisted corpse/phase/customization rows and independent base/auxiliary outcomes | `wow_database::map_corpse_adapter` alone maps that request to `SEL_CORPSES`, `SEL_CORPSE_PHASES` and `SEL_CORPSE_CUSTOMIZATIONS`, preserving exact bind width, order and query failure classification | the transitional `wow-world` application adapter retains corpse validation, item-cache parsing, faction resolution, map-local GUID allocation and publication into canonical `wow_map::Map` | composed once in `world-server`; the map lock is checked before I/O and reacquired only after the complete typed result returns | #392 removes concrete Character-database access from `Map::LoadCorpseData` hydration without folding map state into the Player lifecycle port. #584 owns relocating the transitional Session caller into the complete map-hydration boundary; the canonical corpse owner and clock do not change in that extraction. #153 verifies the completed boundary. |
...
| Effective skill-catalog Hotfix capability | `wow_persistence::SkillCatalogHotfixPersistencePortLikeCpp` owns SQLx-free rows and loaded/failed outcomes for one cross-indexed SkillLine/ability/race-class authority, exposed as dependency-ordered startup stages rather than table CRUD | `wow_database::skill_catalog_hotfix_adapter` alone owns the four Hotfix statements, verified-build binds, concrete handle and checked SQL decoding | `wow_data::{SkillLineStore, SkillStore}` retain WDC4 parsing, overlay precedence, incomplete/invalid diagnostics, tombstones, derived indexes and gameplay queries; private `world-server::skill_catalog_hotfix` converts and composes the typed rows, then builds the immutable TraitMgr projection | SkillLine must publish before relation validation; any stage query/decode failure prevents that store's publication. The adapter completes official/custom `SkillLineAbility` and `SkillRaceClassInfo` in order; the dependent TraitMgr stage loads `TraitTree`/`SkillLineXTraitTree` after effective `SkillRaceClassInfo`, and its class projection resolves `ChrSpecialization -> ClassID` without a generic repository or second authority | #523 removes Hotfix database, statement and result vocabulary from both skill domain modules while preserving the separate TraitMgr consumer boundary and the independent WorldDatabase `SkillTiers` lifecycle. PR #832 adds combat class resolution; remaining node/cost/condition/loadout consumers stay in #524. Exact inventory falls 20,346→20,328 (production 10,038→10,013; fixtures 10,308→10,315), multiplicity 22,651→22,631, direct production falls 3,954→3,853, reviewed adapters rise 3,807→3,880 and composition rises 2,277→2,280. |
...
| Immutable World quest-item capability | `wow_persistence::QuestItemCatalogPersistencePortLikeCpp` owns SQLx-free gameobject and creature quest-item rows behind two independent loaded/failed operations | `wow_database::quest_item_catalog_adapter` alone owns both exact `ORDER BY Idx ASC` World statements, one concrete handle, complete-batch query decoding and checked `u32`/`u8` widths | `wow_data::{GameObjectQuestItemStoreLikeCpp, CreatureQuestItemStoreLikeCpp}` retain template/item validation, per-key insertion order, reports, immutable indices and creature-difficulty fallback; private `world-server::quest_item_catalog` converts, diagnoses and composes both stores | one adapter is composed once. Production preserves C++ and current Rust gameobject → creature order, accepts empty batches, fully decodes each batch and returns no store pair after either failure; no transaction or cross-database atomicity is inferred | #548 removes all 42 concrete production identities from `wow_data::quest_item` without adding `wow-data → wow-persistence`, a generic repository, RPC, process or cache. Exact inventory becomes 20,444 rows (10,090 production and 10,354 fixtures), multiplicity 22,857 (11,758 production and 11,099 fixtures); direct production falls 3,196→3,154, reviewed adapters rise 4,546→4,605 and composition rises 2,329→2,331. Three unstable workflows become eleven stable adapter workflows, leaving 999 workflows (675 stable and 324 unstable) and 1,003 policy groups (675 stable and 328 unstable). |
| Effective LFGDungeons Hotfix capability | `wow_persistence::LfgDungeonsHotfixPersistencePortLikeCpp` owns the complete SQLx-free 35-field `LFGDungeonsEntry` overlay row and one loaded/failed batch outcome | `wow_database::lfg_dungeons_hotfix_adapter` alone owns `SEL_LFG_DUNGEONS`, one Hotfix handle, complete-batch decoding and checked C++ signed/unsigned widths | `wow_data::LfgDungeonsStore` retains WDC4 loading, whole-row replacement by ID and all LFG gameplay queries; private `world-server::lfg_dungeons_hotfix` converts and publishes the effective store | the base DB2 store loads before the single represented Rust `VerifiedBuild > 0` read. Empty success publishes DB2-only state; query/type/range failure publishes no store. C++'s generic DB2 loader can additionally stage custom rows; correcting that pre-existing parity gap is behavior work outside this ownership cut. The later World template/reward reads remain a separate startup fence and capability | #550 removes all 22 concrete production identities from the early `wow_data::lfg` Hotfix overlay without merging the later World connection, adding a generic repository, or adding `wow-data → wow-persistence`. Exact inventory becomes 20,476 rows (10,120 production and 10,356 fixtures), multiplicity 22,929 (11,828 production and 11,101 fixtures); direct production falls 3,154→3,132, reviewed adapters rise 4,605→4,655 and composition rises 2,331→2,333. One unstable Hotfix workflow becomes eight stable adapter workflows while the World module workflow remains open, leaving 1,006 workflows (683 stable and 323 unstable) and 1,010 policy groups (683 stable and 327 unstable). |
| Late LFG World-catalog capability | `wow_persistence::LfgWorldCatalogPersistencePortLikeCpp` owns SQLx-free entrance-template and ordered reward rows behind two independent loaded/failed methods | `wow_database::lfg_world_catalog_adapter` alone owns both World statements, one World handle, complete-batch decoding and checked C++ widths | `wow_data::LfgDungeonStoreLikeCpp` retains DB2 type/MapDifficulty/Quest validation, template application, random grouping, reward correction/reporting and selection; private `world-server::lfg_world_catalog` maps and composes the effective store | production reads templates before `ORDER BY dungeonId, maxLevel ASC` rewards at the existing post-Quest fence. Empty batches succeed; first failure stops before rewards and second failure exposes no partial store. C++ AreaTrigger fallback and reward max-level correction are pre-existing behavior gaps outside this ownership cut | #552 removes the final 41 concrete production identities from `wow_data::lfg`, completing that module's persistence isolation without merging its earlier Hotfix connection or inventing a transaction/repository. Exact inventory becomes 20,498 rows (10,139 production and 10,359 fixtures), multiplicity 22,965 (11,861 production and 11,104 fixtures); direct production falls 3,132→3,091, reviewed adapters rise 4,655→4,713 and composition rises 2,333→2,335. Two unstable workflows become ten stable adapter workflows, leaving 1,014 workflows (693 stable and 321 unstable) and 1,018 policy groups (693 stable and 325 unstable). |
| Battle-pet selection World capability | `wow_persistence::BattlePetSelectionCatalogPersistencePortLikeCpp` owns SQLx-free species/breed and species/quality rows behind two independent loaded/failed methods | `wow_database::battle_pet_selection_catalog_adapter` alone owns both World statements, one World handle, complete-batch decoding and checked C++ widths | `wow_data::battle_pet_selection` retains species validation, breed dedup/sort, quality constraints, C++ defaults and RNG selection; private `world-server::battle_pet_selection_catalog` maps rows and deliberately applies the C++ null-result tolerance | production reads breeds before qualities. Either query/decode failure is logged and collapsed to an empty source without suppressing the other read; empty success stays distinguishable at the port. No transaction or cross-database boundary is claimed | #554 removes all 38 exact concrete production identities (multiplicity 41) from `wow_data::battle_pet_selection`. Exact inventory becomes 20,516 rows (10,154 production and 10,362 fixtures), multiplicity 22,987 (11,880 production and 11,107 fixtures); direct production falls 3,091→3,053, reviewed adapters rise 4,713→4,764 and composition rises 2,335→2,337. Two unstable workflows become seven stable adapter workflows, leaving 1,019 workflows (700 stable and 319 unstable) and 1,023 policy groups (700 stable and 323 unstable). |
| Item random-enchantment World capability | `wow_persistence::ItemRandomEnchantmentCatalogPersistencePortLikeCpp` owns one SQLx-free C++ `uint32/uint32/float` row and loaded/failed complete-batch outcome | `wow_database::item_random_enchantment_catalog_adapter` alone owns `SEL_ITEM_RANDOM_ENCHANTMENT_TEMPLATE`, one World handle and checked row decoding | `wow_data::item_random_enchantment` retains DB2 random-property/suffix validation, chance validation, grouping and accessors; private `world-server::item_random_enchantment_catalog` converts, validates and publishes | one statement is fully decoded before domain mutation. Empty succeeds; failure remains startup-fatal as in pre-#556 Rust. C++ treats a null result as an empty catalog, so changing that pre-existing failure-policy divergence is behavior work outside this ownership cut | #556 removes all 33 concrete production identities from `wow_data::item_random_enchantment`, including both unused/production loader surfaces. Exact inventory becomes 20,532 rows (10,168 production and 10,364 fixtures), multiplicity 23,012 (11,903 production and 11,109 fixtures); direct production falls 3,053→3,020, reviewed adapters rise 4,764→4,809 and composition rises 2,337→2,339. Four unstable workflows become six stable adapter workflows, leaving 1,021 workflows (706 stable and 315 unstable) and 1,025 policy groups (706 stable and 319 unstable). |
| GameTele World capability | `wow_persistence::GameTeleCatalogPersistencePortLikeCpp` owns one SQLx-free C++ `uint32/float/float/float/float/uint16/string` row and loaded/failed complete-batch outcome | `wow_database::game_tele_catalog_adapter` alone owns `SEL_GAME_TELE`, one World handle and checked row decoding | `wow_data::game_tele` retains coordinate validation, Unicode normalization, duplicate-ID replacement, reports and lookups; private `world-server::game_tele_catalog` converts and publishes | the one statement is fully decoded before domain mutation. Empty succeeds and query/decode failure remains startup-fatal as in pre-#558 Rust. C++ treats a null result as empty and skips invalid UTF-8 names, while current Rust fails the startup read and lowercases a successfully decoded Rust string; both pre-existing behavior differences remain outside this boundary-only slice | #558 removes all 25 concrete production identities from `wow_data::game_tele`. Exact inventory becomes 20,559 rows (10,193 production and 10,366 fixtures), multiplicity 23,051 (11,940 production and 11,111 fixtures); direct production falls 3,020→2,995, reviewed adapters rise 4,809→4,857 and composition rises 2,339→2,341. Two unstable workflows become seven stable adapter workflows, leaving 1,026 workflows (713 stable and 313 unstable) and 1,030 policy groups (713 stable and 317 unstable). |
| Exploration BaseXP World capability | `wow_persistence::ExplorationBaseXpCatalogPersistencePortLikeCpp` owns one SQLx-free C++ `uint8/uint32` row and loaded/failed complete-batch outcome | `wow_database::exploration_base_xp_catalog_adapter` alone owns `SEL_EXPLORATION_BASE_XP`, one World handle and the C++ signed-`int32` to `uint32` conversion while retaining the pre-existing unsigned compatibility/fallback shape | `wow_data::xp` retains the level map, duplicate replacement, missing-level zero and every exploration-reward branch; private `world-server::exploration_base_xp_catalog` converts, logs and publishes | the one statement is fully decoded before domain mutation. Empty succeeds and query failure remains startup-fatal as in pre-#562 Rust. C++ treats a null result as an empty catalog, so changing that pre-existing failure-policy difference remains outside this boundary-only slice | #562 removes all 31 concrete production identities (multiplicity 32) from `wow_data::xp`. Exact inventory becomes 20,563 rows (10,193 production and 10,370 fixtures), multiplicity 23,056 (11,941 production and 11,115 fixtures); direct production falls 2,995→2,964, reviewed adapters rise 4,857→4,886 and composition rises 2,341→2,343. Three unstable workflows become six stable adapter workflows, leaving 1,029 workflows (719 stable and 310 unstable) and 1,033 policy groups (719 stable and 314 unstable). |
| Reserved-player-name Character capability | `wow_persistence::ReservedNameCatalogPersistencePortLikeCpp` owns one SQLx-free string row and a loaded/failed complete-batch outcome | `wow_database::reserved_name_catalog_adapter` alone owns `SEL_RESERVED_NAMES`, one Character handle and row decoding | `wow_data::reserved_names` retains lowercase normalization, duplicate-row accounting, immutable set ownership and case-insensitive lookup; private `world-server::reserved_name_catalog` converts and publishes | the one statement is fully decoded before domain mutation. Empty succeeds and query/decode failure remains startup-fatal as in pre-#564 Rust. C++ treats a null result as empty and skips invalid UTF-8 names, while current Rust's required string decode fails the startup read; correcting that pre-existing difference remains outside this boundary-only slice | #564 removes all 22 concrete production identities from `wow_data::reserved_names`. Exact inventory becomes 20,566 rows (10,194 production and 10,372 fixtures), multiplicity 23,060 (11,943 production and 11,117 fixtures); direct production falls 2,964→2,942, reviewed adapters rise 4,886→4,907 and composition rises 2,343→2,345. Two unstable workflows become four stable adapter workflows, leaving 1,031 workflows (723 stable and 308 unstable) and 1,035 policy groups (723 stable and 312 unstable). |
| Jump-charge World capability | `wow_persistence::JumpChargeCatalogPersistencePortLikeCpp` owns one SQLx-free seven-field row with explicit nullable SpellVisual/progress/parabolic references and a loaded/failed complete-batch outcome | `wow_database::jump_charge_catalog_adapter` alone owns `SEL_JUMP_CHARGE_PARAMS`, one World handle and exact signed/float/bool/null decoding | `wow_data::jump_charge` retains speed/gravity defaults, SpellVisual/Curve validation, the C++ progress-curve log typo report, duplicate-ID replacement and immutable lookup; private `world-server::jump_charge_catalog` converts and publishes against the already-loaded DB2 stores | the one statement is fully decoded before domain validation or mutation. Empty succeeds and query/required-field decode failure remains startup-fatal as in pre-#566 Rust. C++ treats a null query result as an empty catalog; correcting that pre-existing difference remains outside this boundary-only slice | #566 removes all 22 concrete production identities from `wow_data::jump_charge`. Exact inventory becomes 20,569 rows (10,195 production and 10,374 fixtures), multiplicity 23,064 (11,945 production and 11,119 fixtures); direct production falls 2,942→2,920, reviewed adapters rise 4,907→4,928 and composition rises 2,345→2,347. Two unstable workflows become four stable adapter workflows, leaving 1,033 workflows (727 stable and 306 unstable) and 1,037 policy groups (727 stable and 310 unstable). |
| GameEvent startup World capability | `wow_persistence::GameEventWorldCatalogPersistencePortLikeCpp` owns eleven SQLx-free row families and two loaded/failed batches: the World prefix and suffix separated by the independent Character condition-save overlay | `wow_database::game_event_world_catalog_adapter` alone owns the fourteen World statements, one World handle, signed event-id reinterpretation and checked integer/null/string decoding | `world-server::spawn_store_loader` retains GameEvent sizing, stores, reports, PoolMgr/SpawnStore validation, publication and every scheduler/runtime consumer; the existing Character mutation/load port remains independent | production preserves the represented Rust fence `World prefix -> Character saves -> World suffix`; each World stage is fully decoded before domain application, a failure stops later reads and no partially built catalog is published. Empty-result and startup-fatal query/decode policies remain as before; the known C++/Rust load-order and missing-behavior differences remain explicit parity work | #568 removes 460 exact (524 multiplicity) concrete composition identities from `spawn_store_loader` and adds 78 production adapter plus eight composition identities. Exact inventory becomes 20,198 rows (9,821 production and 10,377 fixtures), multiplicity 22,659 (11,537 production and 11,122 fixtures); production composition falls 2,347→1,895, reviewed adapters rise 4,928→5,006 and direct production stays 2,920. Sixteen unstable workflows become seventeen stable adapter workflows, leaving 1,034 workflows (744 stable and 290 unstable) and 1,038 policy groups (744 stable and 294 unstable). |
| Canonical spawn and WorldState startup capabilities | `wow_persistence::CanonicalSpawnCatalogPersistencePortLikeCpp` owns the staged SQLx-free spawn/waypoint/formation/linked-respawn/pool/spawn-group rows and `WorldStateStartupPersistencePortLikeCpp` owns the complete World-template then Character-saved-value catalog | `wow_database::canonical_spawn_catalog_adapter` alone owns the remaining thirteen World statement executions, one World handle, pool type binds and row decoding; `world_state_startup_adapter` alone owns the World and Character handles and their two sequential statements | `world-server::spawn_store_loader` retains SpawnStore, PoolMgr, WorldStateMgr, map/DB2/equipment/phase/script validation, reports and publication; every runtime consumer remains unchanged | production preserves creature -> waypoint -> formation -> gameobject -> area-trigger -> linked-respawn -> pool templates -> Creature/GameObject/Pool members -> pool validation -> autospawn -> GameEvent -> spawn-group order, plus the independent WorldState World -> Characters fence. Each stage returns a complete typed batch or fails before later reads; no transaction or distributed atomicity is fabricated | #570 removes 509 exact composition identities (559 multiplicity) and adds 147 production reviewed-adapter identities (162 multiplicity), with direct production unchanged. Exact inventory becomes 19,851 rows (9,459 production and 10,392 fixtures), multiplicity 22,279 (11,140 production and 11,139 fixtures); production composition falls 1,895→1,386, reviewed adapters rise 5,006→5,153 and direct production stays 2,920. All seventeen unstable `spawn_store_loader` workflows retire into thirty stable adapter workflows, leaving 1,047 workflows (774 stable and 273 unstable) and 1,051 policy groups (774 stable and 277 unstable). |
| Complete spell-acquisition startup capability | `wow_persistence::SpellAcquisitionStartupPersistencePortLikeCpp` owns SQLx-free typed outcomes for seven Hotfix overlay families, SpellReagents, server-side spells/effects, custom attributes, `spell_learn_spell`, and trainer audit rows | `wow_database::spell_acquisition_startup_adapter` alone owns the Hotfix/World handles, raw overlay SQL, World statements, binds, tolerant row decoding and read-failure classification | `wow-data` retains WDC parsing, base -> official -> custom -> RecordRemoved composition, validation and immutable catalogs; `world-server::spell_acquisition_loader` retains the exact cross-store startup order, publication points and trainer authority | official and custom Hotfix reads remain sequential on one Hotfix handle; World reads remain sequential on one World handle; the two connections are independent and no distributed transaction is claimed. Empty success remains distinct from failure, and every failed stage stops before later reads or publication | #572 removes 120 production composition and 152 direct identities while adding 150 reviewed-adapter identities; production falls 9,459→9,337 exact rows and 11,140→11,124 multiplicity. Fixture coverage adds eight adapter identities, so the complete inventory becomes 19,737 exact rows and 22,271 multiplicity. Twenty-five unstable workflows retire into eighteen stable adapter workflows, leaving 1,040 workflows (792 stable and 248 unstable) and 1,044 policy groups (792 stable and 252 unstable). |
| Static-data startup capabilities | `wow_persistence` owns cohesive SQLx-free typed ports for every remaining `wow-data` World/Hotfix source, including staged quest rows | `wow_database` alone owns the concrete handles, statements, raw SQL and tolerant row decoding | `wow-data` retains parsing, validation, replacement rules, diagnostics and immutable store construction; private `world-server` bridges convert rows and preserve startup sequencing | each capability is composed once. Multi-stage catalogs keep their prior query/application order and fail before every later read; no partial store is published and no cross-connection transaction or default value is invented | #574 removes the final 24-source `wow-data -> wow-database` dependency and its P4 exception as one complete boundary cut. Direct production identities fall 2,768→1,999 (all 769 removed identities are in `wow-data`), reviewed adapters rise 5,303→5,623 and composition rises 1,266→1,357; the complete inventory falls 19,737→19,404 identities and 22,271→21,842 multiplicity. Seventy-three unstable domain workflows retire into 68 stable adapter workflows, leaving 1,035 workflows (860 stable and 175 unstable) and 1,039 policy groups (860 stable and 179 unstable). The staged quest port preserves all eight reads and their diagnostic/application fences; no gameplay, packet or runtime-clock behavior moves. |
...
| Canonical map runtime | `wow_map::MapManager` | canonical global map loop, grid/spawn/respawn paths, explicit selected legacy-result adapters | world-server orchestration and session map/player bridges | process lifetime; canonical loop uses the configured map interval; existing phase structure does not establish complete production C++ admission/phase semantics | #188 records bounded clock declarations. #584 C0/C3 must prove actual execution/barriers and retire each superseded writer with its complete transition. |
...
| Represented player gameplay state | `wow_entities::Player` is the generation-checked owner of the migrated gameplay families, including C++ `Player::_CUFProfiles`, `_equipmentSets`, `_voidStorageItems`, `_specializationInfo`, `mSkillStatus`, guild membership/`m_GuildIdInvited`, `m_trade`, `m_atLoginFlags`, weapon/armor proficiencies, `m_ArenaTeamIdInvited`, represented `m_bgData` context/queue slots, `m_group`/subgroup and `m_groupUpdateSequences`, plus their coherent-load markers; `GroupRegistry` remains the separate authority for the Group and its member slots; remaining transitional families are still explicit `wow_world::WorldSession` fields | migrated handlers query/mutate the exact `PlayerHandle`; group handlers validate the Player reference against `GroupRegistry`; remaining handlers and session update code still own only their uncut families | packet builders, persistence helpers and `PlayerRegistry` summaries consume typed snapshots/outcomes | selected-character lifetime under `wow_map::MapManager`, including explicit detached far-teleport residence; map-locked paths resolve Player-owned context before acquiring the map lock; the former whole-`Player` Session→Map snapshot/write-back bridge is gone and unresolved/stale handles remain unknown rather than fabricating empty profile, saved-set, vault, specialization, skill, guild, trade, capability, battleground or group context | #584 coordinates the remaining family cuts, Map authority and resource decomposition through analyzed complete operations; #153 performs the terminal audit rather than deferring a known ownership bridge. |
| Concrete persistence access | `persistence-access-snapshot.json` inventories exact SQLx and concrete `wow-database` syntax across application, data, instance, composition and adapter code; `persistence-boundary-policy.json` assigns every row exactly once | `wow-database` is the stable concrete adapter; each remaining group names its current capability owner, logical database(s), affinity and open removal/decision issue | handlers, lifecycle, bootstrap/loaders, runtime recovery, tests and publication paths consume concrete outcomes | statement order, connection affinity, commit/rollback/unknown-commit classification, fences, and publication order are observable; cross-database groups explicitly use independent connections and never imply distributed ACID | #186 installed the exact non-growth/stale-exception guard; #187 freezes ordered behavior; #200 earns the SQLx-free Player lifecycle port; #189 moves durable loot coordination; #584 owns the inherited remaining capability/data/auth/instance decisions as complete boundaries, not new per-statement children. Keep legitimate composition and concrete adapter access; remove actual application/domain inversions with their consumers. #153 verifies exhaustive coverage and final classifications. |
...
- exact active handler registration coverage: opcode value/name, `SessionStatus`,
  `PacketProcessing`, handler name and its sole registered thunk; no second dispatch arm;
...
Before authorized publication, use `./tools/validation-v2 final --base origin/3.4.3` on a clean,
committed HEAD plus the macro's explicit acceptance. It is not the daily iteration command.

For issue-metadata, state or dependency-DAG-only reconciliation, run the Python architecture
check/self-test and Rust `session-ownership-check -- check --syntax-only`; keep every mirrored
issue state and ownership assignment consistent. Syntax-only does not validate persistence issue
states: additionally check the preserved workflow/policy `target_issues` against existing open
issues and their stable/non-stable classification, run
`checked_persistence_policy_matches_the_checked_snapshot`, and verify that the workflow, policy
and snapshot files remain unchanged. Such a refresh does not change persistence or bridge
identities and does not by itself require regenerating either exhaustive inventory.
Implementation that changes those surfaces must validate the affected inventories, and terminal
acceptance must include the exhaustive audit. This proportional rule does not waive a failed
state-consistency guard or permit hand-editing generated policy without its reviewed source.

For the metadata-only case, the bounded check over the **preserved** persistence artifacts is:

```bash
PROTOC=/home/ubuntu/.local/protoc/bin/protoc cargo test --release --locked \
  --manifest-path tools/architecture/handler-contract-check/Cargo.toml \
  checked_persistence_policy_matches_the_checked_snapshot
jq -e \
  --slurpfile ledger tools/architecture/architecture-issue-ledger.json \
  --slurpfile workflows tools/architecture/persistence-boundary-workflows.json \
  '($ledger[0].issues | map({key:(.number|tostring),value:.state}) | from_entries) as $states |
   all(.groups[];
     ((.stable_boundary == false or (.target_issues|length)==0)
      and (.source_class != "production" or .stable_boundary or (.target_issues|length)>0)
      and all(.target_issues[]; $states[tostring] == "open")))
   and all($workflows[0].workflows[];
     (((.stable_boundary and (.target_issues|length)==0)
       or (.stable_boundary == false and (.target_issues|length)>0))
      and all(.target_issues[]; $states[tostring] == "open")))' \
  tools/architecture/persistence-boundary-policy.json
```

This checks existing targets and classification consistency, not inventory freshness against
source. It is not a replacement for the exhaustive check when source annotations, workflows,
policy or access identities change. Execute commands with their exit statuses visible.
...
6. Run architecture self-test/check and affected focused evidence. Run `validation-v2 final`
   before authorized publication on clean HEAD, not after every internal helper. Reserve the
   exhaustive `audit` profile for the relevant explicit audit/release acceptance.
...
| `runtime-clock-phase-trace.json` | bounded clock declarations and existing source/test anchors, not proven spawn reachability or phase/lock semantics | `check_architecture.py check` | reviewed by hand against actual callers |
...
**Validate every mirrored state without rescanning unrelated source.** Dependency exceptions
are checked in Python, while the exhaustive Rust check also validates persistence workflow
ownership. #299's Python-only validation left 91 groups targeting a closed issue, repaired by
#341; that failure must remain covered. For a metadata/state/DAG-only ledger refresh, run:
...
Additionally perform the bounded persistence-state/classification check and
`checked_persistence_policy_matches_the_checked_snapshot` described in **Guardrail commands**,
and verify that the workflow, policy and snapshot artifacts are unchanged. Syntax-only alone is
not persistence-state validation. Run the exhaustive Rust inventory when implementation changes
its persistence/bridge surface, or for explicit terminal/release audit; a metadata refresh is not
itself a reason to repeat that scan. Source annotations and their derived policy must still be
reviewed and changed together whenever a semantic classification actually changes.
...
The following implementation notes and counts are historical evidence at their named cutovers,
not an instruction to repeat them or a census of today's tree. Current acceptance follows the
contracts above, the historical #578 checkpoint, current #584 continuation and the source/module-design ratchets.
...
Issue #231 closed the original source/build/configuration block with namespaced typed configuration,
not the wider #99 module product now assigned to #583. Package defaults live in
...
reviewed workflow annotations (1,041 exact semantic groups), and none targets #169. That historical
snapshot assigned 60 open annotations to #153; this is not the current implementation assignment.
Under the current allocation, #584 owns inherited unresolved boundary decisions and #153 verifies
their completed classification. The preserved source annotations and persistence artifacts still
carry that historical target: #584 C4 must review each semantic boundary and reconcile source
annotations plus the derived policy before terminal audit, not blindly retarget or declare all
composition access inverted. This metadata review does not rewrite or certify those inventories.
Reproduce current counts from the artifacts rather than treating
the numbers in this historical paragraph as a refreshed inventory. Four generated-source
...
The ledger is the offline dependency map, not a serial execution queue. Historical
architecture epics and delivered steps remain for provenance; #584 owns current core
coordination, #99 the module product, and external QA #177 is separate.

- Physical modules and semantic ownership are accepted together within each macro.
- #578 and subsequent foundation/repair deliveries are closed, with their scoped evidence.
- The next core priority is #743, followed preferably by independent #735; remaining
  operation, runtime, persistence and physical boundaries are selected under #584.
- #583 delivers the stateful native/Wasm product after required core. #153 independently
  audits accepted core and product; it does not implement known residuals.

C0–C4 are inherited regression/acceptance checkpoints, not a new issue tree. This
deterministic topological display includes completed steps and is checked against the
JSON ledger. #743 appearing before #735 expresses preference, not a dependency:
...
121. #548 — gameobject and creature quest-item tables through one ordered typed World capability;
122. #550 — LFGDungeons DB2 overlay through one typed Hotfix capability;
123. #552 — late LFG template/reward reads through one typed World capability;
124. #554 — battle-pet breed/quality World reads through one typed capability;
125. #556 — item random-enchantment templates through one typed World capability;
126. #558 — GameTele rows through one typed World capability;
127. #562 — exploration BaseXP rows through one typed World capability;
128. #564 — reserved-player-name rows through one typed Character capability;
129. #566 — jump-charge parameter rows through one typed World capability;
130. #568 — staged GameEvent startup World catalogs through one typed capability;
131. #570 — remaining canonical spawn and WorldState startup reads through staged typed capabilities;
132. #572 — complete spell-acquisition Hotfix and World startup sources through typed capabilities;
133. #574 — remove the complete `wow-data -> wow-database` edge through cohesive typed startup catalogs;
134. #189 — durable loot persistence coordination;
135. #192 — runtime/fanout directory consumers;
136. #193 — combat/loot directory consumers;
137. #194 — quest/spell/movement directory consumers;
138. #197 — atomic group invite/create transitions;
139. #198 — atomic group membership/leadership transitions;
140. #199 — Group persistence/publication closure;
141. #195 — social/group session addressing;
142. #196 — PlayerRegistry storage closure;
143. #138 — opaque session-directory relocation;
144. #191 — mailbox protocol relocation;
145. #137 — encapsulated Group owner move;
146. #190 — durable creature-runtime rail relocation;
147. #140 — Session mailbox pump;
148. #252 — retire the temporary PlayerBroadcastInfo gameplay mirror;
149. #182 — logical realm/instance routing;
150. #183 — Session-only phase driver;
151. #184 — login/logout lifecycle modules;
152. #224 — character/loot/quest physical modules;
153. #225 — Map/MapManager physical modules;
154. #226 — Player/Unit physical modules;
155. #227 — packet/spell-data physical modules;
156. #228 — trusted linked external module API;
157. #229 — deterministic external Cargo composition;
158. #230 — agent-neutral module CLI and skeleton;
159. #231 — typed module configuration/fixtures;
160. #270 — retire the four PlayerBroadcastInfo transport endpoints;
161. #359 — single dispatch mechanism for every opcode;
162. #297 — promote the Session kernel to `wow-session`;
163. #378 — classify the remaining Session application modules; inherited implementation stays in the following ownership macro;
164. #578 — closed canonical Player/Map foundation, merged through PR 579.
165. #585 — closed represented session finalization outcomes and retirement supervision, merged through PR 586.
166. #588 — closed deferred player visibility publication with scoped runtime acceptance.
167. #587 — closed represented spell-acquisition application boundary.
168. #589 — closed represented Player cast-request lifecycle, integrated with its predecessors.
169. #716 — restore ownership provenance and exact acceptance after module decomposition.
170. #718 — closed represented quest-reward transaction and operation boundary.
171. #722 — closed named Player operations and separation of hydration.
