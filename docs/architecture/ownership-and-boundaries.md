...
| Reserved-but-empty crates | `wow-achievement`, `wow-combat`, `wow-pvp`, `wow-spell` (`domain-runtime`) and `wow-ecs` (`foundation`) contain no code | nothing writes them yet | nothing reads them: after #288 no crate declares a dependency on any of them | none — they hold no state and run on no clock | Names reserved for planned work: `wow-spell` for the M3 spell issues (#30–#35), `wow-combat` for #29, `wow-pvp` and `wow-achievement` for later parity, `wow-ecs` for an entity-storage decision not yet taken. #288 removed the five phantom `wow-ecs` dependency edges that made an empty crate look like a real boundary and inflated every fan-out figure. Whoever fills one owns turning it into a real boundary; until then they must not accumulate dependents. |
| Player lifecycle persistence capability | `wow_persistence`: `PlayerOfflineMarkLikeCpp`, `LogicalDatabaseLikeCpp` and the three-way `PersistenceOutcomeLikeCpp`, behind `PlayerLifecyclePortLikeCpp`. The crate has **no dependencies at all** — no pool, row, transaction, statement or SQL string | `wow_database::player_lifecycle_adapter` is the only implementation and the only place that knows the statement, the pool and the driver error | `wow_world` Session lifecycle holds the port, never a database handle for these writes | composed in `world-server` before any session is accepted, so a build that cannot persist lifecycle state fails at startup rather than dropping offline marks at logout | #200 earned the crate with the offline-mark vertical: three production writes across two logical databases. 200b moves the character save plan and 200c the account collections; the order they must preserve is frozen in `player-lifecycle-persistence-contract.md` (#187). |
...
| Logical realm/instance connection | private `wow_world::session::connection`: the attach/switch/restore state machine, realm-vs-primary send selection and the two cross-socket ordering fences | the owning Session task, which is the only writer of the logical primary | `send_packet_realm`/`send_raw_packet_realm` callers, the Session driver polling the instance link, and logout restore | connection lifetime; after `SMSG_CONNECT_TO` the instance channel becomes primary and the realm channel is parked, never closed — the client drops the session if either socket dies | #182 extracted the transition implementation out of `session/mod.rs`. The physical channels and fences stay on `WorldSession` fields for now; #252 and #270 retire that storage. |
