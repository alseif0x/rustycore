...
| Hotfix delivery-metadata capability | `wow_persistence::HotfixDeliveryMetadataPersistencePortLikeCpp` owns SQLx-free blob, push/data and optional-data rows with three separately invoked loaded/failed stages | `wow_database::hotfix_delivery_metadata_adapter` alone owns the three exact Hotfix statements, concrete handle, binary/string reads and checked C++-width integer decoding | `wow_data::HotfixBlobCache` retains local WDC4 record discovery, locale admission, known-store/blob filtering, push grouping and optional-data indexing; private `world-server::hotfix_delivery_metadata` converts and applies each stage | one adapter is composed once after local DB2 discovery. Blob, data and optional-data reads/applications remain ordered but independently warn-and-continue; a failed stage cannot partially mutate its domain state and does not prevent later stages | #534 removes all 62 concrete production identities from `wow_data::hotfix_cache` without an eager table bundle, repository, RPC or new process. Exact inventory becomes 20,393 rows (10,061 production and 10,332 fixtures), multiplicity 22,753 (11,676 production and 11,077 fixtures); direct production falls 3,626→3,564, reviewed adapters rise 4,110→4,180 and composition rises 2,291→2,317. Four unstable workflows are retired and fourteen stable adapter workflows are added, leaving 952 workflows (597 stable and 355 unstable) and 956 policy groups (597 stable and 359 unstable). This ownership-only cut explicitly does not claim C++ parity for all-locale masks, typed `DB2StorageBase::WriteRecord`, blob/store diagnostics or optional-key/data validation. |
