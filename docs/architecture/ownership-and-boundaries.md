...
| Core SpellInfo Hotfix DB2 capability | `wow_persistence::SpellCoreDb2HotfixPersistencePortLikeCpp` owns SQLx-free typed rows and loaded/failed outcomes for SpellName, Categories, Misc, Effect, Shapeshift, Interrupts, CastTimes, Cooldowns, CastingRequirements, Power and PowerDifficulty | `wow_database::spell_core_db2_hotfix_adapter` alone owns the eleven Hotfix statements, official/custom bind order, concrete handle, tolerant row decoding and query-failure classification | `wow_data::spell_db2` retains WDC4 parsing, record replacement and final `hotfix_data` tombstones; `wow_data::SpellStore` retains derived hydration; private `world-server::spell_core_db2_hotfix` converts boundary DTOs and composes the effective stores | each family preserves DB2 → official → custom → final tombstones; startup stays SpellName, the pre-existing out-of-scope key seed, Categories, Misc, Effect, Shapeshift, Interrupts, CastTimes, Cooldowns, CastingRequirements, Power and PowerDifficulty before hydration. Query failure remains fatal before affected publication | #509 removes 236 net direct production accesses from the selected `wow-data` loaders without adding the forbidden `wow-data → wow-persistence` edge. The exhaustive inventory falls to 20,639 exact rows (10,338 production and 10,301 fixtures); direct production access falls 4,796→4,560, reviewed-adapter access rises 3,436→3,526 and composition rises 2,246→2,252. Thirteen old direct workflows become one explicit out-of-scope key-seed workflow plus sixteen stable adapter workflows, leaving 904 workflows (489 stable and 415 unstable). |
