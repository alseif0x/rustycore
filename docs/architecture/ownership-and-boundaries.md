...
| Foundational SpellMgr World-catalog capability | `wow_persistence::SpellWorldCatalogPersistencePortLikeCpp` owns SQLx-free typed rows and loaded/failed outcomes for SpellRequired, SpellThreat, SpellLinked, SpellTotemModel and SpellPetAura | `wow_database::spell_world_catalog_adapter` alone owns the five exact World statements, concrete handle and existing tolerant numeric row decoding | `wow_data::spell::stores` retains spell/effect/rank/race/display validation, duplicate handling, warnings and immutable catalogs; private `world-server::spell_world_catalog` converts boundary DTOs and preserves the startup calls | every table remains an independent non-transactional World read; empty results are successful empty batches, query failure remains fatal before the affected publication, and the pre-cut startup order is unchanged | #515 removes 115 direct production accesses from the five simple loaders without folding the remaining complex SpellArea/Group/StackRule/Proc/TargetPosition reads into the same PR. Exact inventory falls 20,501→20,449 (production 10,200→10,145; fixtures 10,301→10,304), multiplicity 22,800→22,748, direct production falls 4,301→4,186, reviewed adapters rise 3,637→3,691 and composition rises 2,262→2,268. Five unstable workflows become eleven stable adapter workflows, leaving 910 workflows (516 stable and 394 unstable) and 914 policy groups (516 stable and 398 unstable). |
