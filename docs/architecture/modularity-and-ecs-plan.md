...
**Plan synchronization, 2026-09-15 (#953 / #950 / #948 / #936 / #935 / #933 / #931 / #929 / #927 / #925 / #924 / #923 / #922 / #921 / #919 / #918 / #917 / #916 / #915 / #913 / #911 / #909 / #907 / #906 / #904 / #902 / #901 / #899 / #897 / #895 / #891 / #889 / #887 / #881 / #878 / #876 / #871 / #866 / #864 / #862 / #860 / #859 / #855 / #854 / #853 / #851 / #848 / #846 / #844 / #808 / #748):** `PORT_PLAN.md` and GitHub #49 are the
...
The current code integration head is `507f3cfa7c34bc96a5173b107929fa6ff21ac0d4` (PR #953, following PR #948/#935/#933/#931/#929/#927/#926/#925/#924/#923, PR #922/#921, PR #918, PR #917, PR #916, PR #915, PR #913, PR #909, PR #907, PR #906, PR #904, PR #902, PR #901, PR #899, PR #897, PR #895, PR #893 and PR #891,
...
`SkillRaceClassInfo` WDC4/SQL startup sequence, including official/custom
`SkillLineXTraitTree` hotfix overlays plus table-hash-scoped final removals, and PR #810 installs the immutable
`SkillLineXTraitTree` profession projection through the production session capability
boundary. PR #830 also installs the immutable generic `TraitSystemID -> TraitTreeID`
...
also rejects missing or over-ranked persisted `TraitNodeEntry` rows at login. PR #842
(`995cd77f`, implementation `add6650a`) now performs C++-aligned semantic condition, cost,
parent/rank and granted-entry fallback validation before login publication from canonical
Player facts. PR #846 (`93fa95a9`, implementation `57116f75`) now composes all 24
C++-projected base Trait/`SpecSetMember` SQL hotfix tables with official-then-custom
precedence, WDC4-hash-scoped removals and fail-before-publication checks. PR #848
(`179fd5d4`, implementation `95274da1`) adds the `TraitDefinition` and
`TraitCurrencySource` locale overlays with exact SQL projections and immutable Player
capability retention. #524 remains open for explicit cross-store coverage,
...
PR #909 integrates the next C0–C4 classification of `player_guid` into a dedicated
...
The fresh post-#929 audit selected `crates/world-server/src/creature_loaded_grid.rs`
for a physical split. Its 797-line production facade now mounts the shared
`creature_loaded_grid_tests/mod.rs` fixture facade and the responsibility-scoped
`builder.rs` and `resolver.rs` children, retaining all 28 regressions under explicit
`cfg(test)`. No authority, dependency, runtime, packet or persistence behavior
...
## P4 spawn catalog navigability — integrated PR #935, 2026-09-15, merge `c922fd60`

The fresh post-#933 audit selected the 5,125-line `spawn_store_loader.rs` hotspot
for a responsibility split. PR #935 keeps startup composition, DB ownership and
spawn conversion in the facade while moving GameEventMgr catalog models to
`spawn_store_loader/game_event_catalog.rs` and WorldStateMgr startup/index rules to
`spawn_store_loader/world_state_catalog.rs`. The resulting files are 3,427, 1,355
and 345 lines, with deliberate reexports preserving the existing public paths. The
runtime child visibility correction restores the published #933 parent reexport
contract. The focused game-event suite passes 181/181; `cargo check`, fmt/diff and
architecture check/self-test pass. The final local profile still reports three
pre-existing GameObject respawn-save failures in `scenarios_9` (587 passed, 3 failed).
Follow-up PR #936 tightens both physical policy rows to the exact scanner counts at the
...
reconciles the stale Session ownership anchor, exact syntax baseline, registry
rows and bridge rows caused by #929/#933; its checker-only suite passes 364/364,
...
following `PlayerTaxi::NextTaxiDestination` (`PlayerTaxi.h:74`) and
`Player::CleanupAfterTaxiFlight` (`Player.cpp:22019`). Session remains the map,
spline and packet/application boundary, and the handle-less generic taxi adapter is
fixture-only under `cfg(test)`. The Player owner regression, 14 taxi tests, 11
canonical-access scenarios, 13 movement scenarios, package checks and architecture
ratchet pass. Route generation, node admission, teleport ordering, persistence,
captures and live QA remain separate gameplay gates under #584.
...
PR #864 completes the following movement-force ACK boundary from
`MovementHandler.cpp:581-663`. Apply, Remove and mod-magnitude ACKs now compare
the status GUID with the active `m_unitMovedByMe`, read the final expected magnitude
from that Unit, adjust the accepted client time and publish from the active mover's
position/GUID. The Session remains an adapter: map lookup is released before the
visibility send, and no packet is delivered under a map lock. A controlled-mover
regression covers the three opcodes and a wrong-GUID refusal; 50 movement-handler
tests pass. Ordinary speed ACKs, knockback, remaining transport/death/BG/taxi
branches, broader mover kinds, exact captures and live QA remain explicit #63
boundaries.

PR #866 completes the following knockback-ACK admission boundary from
`MovementHandler.cpp:548-559`. The handler performs Player-owned validation but
admits the status when its GUID is the active `m_unitMovedByMe`; accepted movement
info remains owned by the Player and `MoveUpdateKnockBack` is published from the
Player source, matching the C++ path. The controlled-mover regression and 50-test
movement suite pass, with architecture, `world-server` and `validation-v2 quick`
evidence recorded at the integration candidate. Ordinary speed ACKs, remaining
...
PR #878 closes the adjacent P2 owner surface for the Player mount VehicleKit. The
named operations in `wow-entities::Player` cover installation, snapshot, clear,
uninstall/removal and ejectable-passenger removal, matching
`Unit::CreateVehicleKit`, `Unit::RemoveVehicleKit` and `Unit::GetVehicleKit`
(`Unit.cpp:11304-11323`). Session remains responsible for vehicle-template
admission and aura, packet and presentation effects; detached vehicle fixtures
are test-only. The owner lifecycle regression, three Session eject regressions,
package checks and architecture ratchet pass. This does not widen the vehicle
CREATE/DESTROY, seat/offset, passenger, Pet/corpse/Transport, persistence or live
QA boundaries; the next #584 macro still requires a fresh responsibility audit.
