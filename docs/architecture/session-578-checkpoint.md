...
phase visibility as the next bounded macro; PR #901 integrates it at
...
PR #820 integrates this bounded #584 delivery into `3.4.3` as
`62c1369f4e49200b6f6d7605b0bc7de5caeace7c` (implementation `8ab62574`). TrinityCore's
...
## P3.11 canonical Pet visibility CREATE — 2026-09-14, #584 / PR #925, integration `a1f66c33`

The canonical map indexes `AccessorObjectKind::Pet` in the same Creature cell
family used by TrinityCore's generic `Map::AddToMap` unit path. PR #925 changes
`visible_creatures_from_canonical_map_like_cpp` to read that entry through
`with_creature_or_pet_like_cpp`; an in-world Pet therefore reaches the existing
`WorldCreature::create_data_from_canonical_like_cpp` and Creature CREATE block after
the normal map, phase, range and detection gates. The source anchors are
`Map.cpp:530-610` (`UpdateObjectVisibilityOnCreate`) and `Pet.cpp:69-88`
(`Pet::AddToWorld`/`Unit::AddToWorld`).
...
`Pet::RemoveFromWorld` reaches that Unit path (`Pet.cpp:94-101`). Rust now captures
eligible Creature/Pet recipients before canonical erasure and publishes the existing
deferred directed DESTROY command for Pet GUIDs. The charmer exclusion,
map-incarnation fence and Session `HaveAtClient` check remain unchanged; Pet AI,
summon ownership, persistence, vehicle/transport lifecycle and live QA remain
separate gates.
...
The finite #584 macro restored the order of the represented spawn phase in
...
creatures, out-of-range aura casters, summons and every active non-player source
(`Maps/Map.cpp:701-754`). `ObjectUpdater::Visit<T>` updates only in-world objects in
those visited cells (`Grids/Notifiers/GridNotifiers.cpp:258-264`); Players and Corpses
